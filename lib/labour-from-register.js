...
export function applyTradeHoursReviewToLabourSummary(reviewRows, { makeKey } = {}) {
  ...
  if (list.length === 0) {
    return {
      ...
      message: 'Add at least one trade, then tap Apply to add them to the labour summary.',
      rows: [],
    }
  }
  ...
  if (unresolved.length > 0) {
    return {
      ...
      reason: 'needs-review',
      message:
        'Resolve every trade and hours marked for review before applying to the labour summary.',
      rows: [],
  ...
  if (!rows.length) {
    return {
      ...
      message: 'Those rows could not be added to the labour summary. Check each trade, then try again.',
      rows: [],
    }
  }
...
export function applyOperativesToLabourSummary(operatives, { groupBy = 'trade_company', makeKey } = {}) {
  ...
  if (included.length === 0) {
    return {
      ok: false,
      reason: 'none-selected',
      message: 'Select at least one operative, then tap Apply to add them to the labour summary.',
      rows: [],
    }
  }
  ...
  if (!rows.length) {
    return {
      ok: false,
      reason: 'empty-aggregate',
      message: 'Those rows could not be added to the labour summary. Check trade, company, and sign-in times, then try again.',
      rows: [],
    }
  }
  return {
...
export function matchedIncludedSignInOperatives(operatives = []) {
  return (Array.isArray(operatives) ? operatives : []).filter(
    (row) => row && typeof row === 'object' && row.dateStatus === 'match' && row.included !== false,
  )
}
