...
describe('Claude OCR benchmark row qualification', () => {
  it('A: rejects a normal table header row (Date/Name/Company/Trade/Time in/Time out/Signature)', () => {
    assert.equal(
      isClaudeBenchmarkHeaderRow({
        date: 'Date',
        person_name: 'Name',
  ...
  it('B: still rejects the header when Trade is corrupted to Painter', () => {
    assert.equal(
      isClaudeBenchmarkHeaderRow({
        date: 'Date',
        person_name: 'Name',
    ...
    const qualified = qualifyClaudeBenchmarkOperativeRows([
      ...
      },
      {
        source_row: 2,
        date: '2026-09-06',
        person_name: 'Real Worker',
        company: 'EDU Painting',
        trade: 'Painter',
        time_in: '08:00',
        time_out: '15:30',
      },
    ...
    assert.equal(qualified.rows[0].person_name, 'Real Worker')
    assert.equal(qualified.rows[0].source_row, 1)
  })

  it('C: still rejects the header when one or two header labels are blank', () => {
    assert.equal(
      isClaudeBenchmarkHeaderRow({
        date: 'Date',
        person_name: '',
  ...
  it('D: keeps a genuine operative with blank/illegible name + company + valid times', () => {
    assert.equal(
      isClaudeBenchmarkHeaderRow({
        date: '2026-09-06',
        person_name: '',
    ...
    assert.equal(qualified.rows[0].company, 'SHC')
    assert.equal(qualified.rows[1].person_name, 'illegible')
  })

  it('E: keeps a genuine painter whose trade is Painter', () => {
    assert.equal(
      isClaudeBenchmarkHeaderRow({
        date: '2026-09-06',
        person_name: 'A',
    ...
    assert.equal(qualified.droppedHeaderCount, 0)
    assert.equal(qualified.rows.length, 1)
    assert.equal(qualified.rows[0].trade, 'Painter')
  })

  it('F: does not reject a genuine worker merely because one field equals Trade or Name', () => {
    assert.equal(
      isClaudeBenchmarkHeaderRow({
        date: '2026-09-06',
        person_name: 'Alex Walker',
        company: 'SHC',
        trade: 'Trade',
        time_in: '08:00',
        time_out: '16:00',
  ...
  it('G: existing normal header rejection remains green; real operative is not a header', () => {
    assert.equal(
      isClaudeBenchmarkHeaderRow({
        date: 'Date',
        person_name: 'Name',
  ...
  it('H: benchmark fixture yields 14 genuine operatives when the table header is present', () => {
    const genuine = []
    for (let i = 0; i < 4; i++) {
      genuine.push({
        source_row: i + 1,
        date: '2026-09-05',
        person_name: `Worker ${i + 1}`,
        company: 'SHC',
        trade: 'Labourer',
        time_in: '08:00',
        time_out: '16:00',
    ...
    for (let i = 0; i < 10; i++) {
      genuine.push({
        source_row: i + 5,
        date: '2026-09-06',
        person_name: `Worker ${i + 5}`,
        company: i < 4 ? 'SHC' : i < 6 ? 'L+L Contracts' : i < 8 ? 'Pro Plumbing' : 'EDU Painting',
        trade: i >= 8 ? 'Painter' : 'Labourer',
        time_in: '08:00',
        time_out: '16:00',
      })
    }
    assert.equal(genuine.length, 14)
    const withTableHeader = [
      {
        source_row: 0,
        date: 'Date',
        person_name: 'Name',
    ...
    const qualified = qualifyClaudeBenchmarkOperativeRows(withTableHeader)
    assert.equal(qualified.droppedHeaderCount, 1)
    assert.equal(qualified.rows.length, 14)
    assert.notEqual(qualified.rows.length, 15)
  })
...
describe('Claude OCR benchmark Company/Trade consensus merge', () => {
  ...
  it('trade null-raw accepts consensus normalized trade when needs_review is false', () => {
    const field = resolveClaudeBenchmarkTradeReviewField(null, {
      normalized_value: 'Electrician',
      needs_review: false,
      confidence: 0.92,
    })
    assert.equal(field.raw_value, null)
    assert.equal(field.reviewed_value, 'Electrician')
    assert.equal(field.needs_review, false)
    assert.equal(field.confidence, 0.92)
  })

  it('trade null-raw rejects consensus when needs_review is true even with normalized_value', () => {
    const field = resolveClaudeBenchmarkTradeReviewField(null, {
      normalized_value: 'Electrician',
      needs_review: true,
      confidence: 0.4,
    })
    assert.equal(field.reviewed_value, null)
    assert.equal(field.needs_review, true)
  })

  it('trade null-raw stays unresolved when consensus normalized is empty', () => {
    const field = resolveClaudeBenchmarkTradeReviewField(null, {
  ...
  it('trade non-null first pass still uses identity review rules', () => {
    const field = resolveClaudeBenchmarkTradeReviewField('Scaff', {
      normalized_value: 'Sparky',
      needs_review: false,
      confidence: 0.88,
    })
    assert.equal(field.raw_value, 'Scaff')
    assert.equal(field.reviewed_value, 'Sparky')
    assert.equal(field.needs_review, false)
  })
...
describe('Claude OCR benchmark Spark/Sparky trade normalisation', () => {
  ...
  it('leaves Electrician and unrelated unknown trades unchanged (no fuzzy rewrite)', () => {
    ...
    assert.equal(normalizeClaudeBenchmarkTradeValue('Zzxq'), 'Zzxq')
    assert.equal(normalizeClaudeBenchmarkTradeValue(null), null)
  })

  it('maps Scaff to Scaffolder without mapping to Electrician', () => {
    assert.equal(normalizeClaudeBenchmarkTradeValue('Scaff'), 'Scaffolder')
    assert.notEqual(normalizeClaudeBenchmarkTradeValue('Scaff'), 'Electrician')
  })
