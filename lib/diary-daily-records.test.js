...
describe('daily records — Site Diary page wiring + order', () => {
  ...
  it('workbench section order matches accepted Site Diary layout', () => {
    const weather = diaryPage.indexOf('title="Weather"')
    const labour = diaryPage.indexOf('<SiteDiaryLabourSection')
    const visitors = diaryPage.indexOf('title="Visitors"')
    const daily = diaryPage.indexOf('<DiaryDailyRecordSections')
    const plant = diaryPage.indexOf('title="Plant"')
    ...
    const delays = diaryPage.indexOf('title="Delays & issues"')
    const actions = diaryPage.indexOf('title="Actions required"')
    const photos = diaryPage.indexOf('<PhotoWorkspace')
    const summary = diaryPage.indexOf('title="Site summary"')
    ...
    assert.ok(
      weather > 0
        ...
        && plant > daily
        && equipment > plant
        && temporaryWorks > equipment
        && delays > temporaryWorks
        && actions > delays
        && photos > actions
        && summary > photos
        && signature > summary,
