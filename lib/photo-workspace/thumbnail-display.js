...
import {
  ...
  recordDisplaySignedUrlBatchSigned,
  storeDisplaySignedUrl,
} from './display-signed-url-cache.js'

export {
  DISPLAY_SIGNED_URL_BUCKET,
...
/**
 * Synchronously attach cached display URLs to grid rows (no Supabase calls).
 * Thumb path preferred; never promotes report.jpg when thumbnail_path exists.
 * @param {object[]} rows
 */
export function enrichGridPhotoRowsFromDisplayCache(rows = []) {
  const list = Array.isArray(rows) ? rows : []
  return list.map((row) => {
    const thumbPath = String(row.thumbnailPath || row.thumbnail_path || '').trim()
    const reportPath = String(row.storagePath || row.url || row.imageUrl || '').trim()
    let thumbnailPreview = row.thumbnailPreview ?? null
    let preview = row.preview ?? null

    if (thumbPath && !isBrowserDisplaySrc(thumbnailPreview)) {
      const cached = getValidDisplaySignedUrl(thumbPath, { recordStats: true })
      if (cached) thumbnailPreview = cached
    } else if (!thumbPath && reportPath && !isBrowserDisplaySrc(preview)) {
      const cached = getValidDisplaySignedUrl(reportPath, { recordStats: true })
      if (cached) preview = cached
    }

    if (thumbnailPreview === (row.thumbnailPreview ?? null) && preview === (row.preview ?? null)) {
      return row
    }
    return { ...row, thumbnailPreview, preview }
  })
}
...
export function createPhotoDisplaySignSession(opts = {}) {
  ...
  function peek(path) {
    ...
    const tabCached = getValidDisplaySignedUrl(key, { recordStats: false })
    if (tabCached) return tabCached
    const hit = cache.get(key)
    return hit && typeof hit.then !== 'function' ? hit : null
  }

  function seedSessionFromDisplayCache(path) {
    const tabCached = getValidDisplaySignedUrl(path, { recordStats: true })
    if (tabCached) cache.set(path, tabCached)
    return tabCached
  }
  ...
  async function resolveOne(path) {
    ...
    if (!key) return null
    if (isBrowserDisplaySrc(key)) return key
    const tabCached = seedSessionFromDisplayCache(key)
    if (tabCached) return tabCached
    if (cache.has(key)) {
      const hit = cache.get(key)
    ...
    const pending = (async () => {
      if (typeof singleSignPath === 'function') {
        ...
        const trimmed = isBrowserDisplaySrc(url) ? String(url).trim() : null
        if (trimmed) storeDisplaySignedUrl(key, trimmed, expiresIn)
        return trimmed
      }
      if (typeof batchSignPaths !== 'function') return null
      batchApiCalls += 1
      recordDisplaySignedUrlBatchSigned(1)
      const rows = await batchSignPaths([key], expiresIn)
      const mapped = mapSignedUrlResults(rows, [key])
      const signed = mapped.get(key) || null
      if (signed) storeDisplaySignedUrl(key, signed, expiresIn)
      return signed
    })().catch(() => null)
  ...
  async function resolveMany(paths) {
    const allPaths = dedupeStoragePaths(paths)
    for (const p of allPaths) {
      seedSessionFromDisplayCache(p)
    }
    const needed = allPaths.filter((p) => {
      if (getValidDisplaySignedUrl(p, { recordStats: false })) {
        seedSessionFromDisplayCache(p)
        return false
      }
    ...
    if (!needed.length) {
      const out = new Map()
      for (const p of allPaths) {
        const v = cache.get(p) || getValidDisplaySignedUrl(p, { recordStats: false })
    ...
    for (const chunk of chunkPaths(needed, batchSize)) {
      batchApiCalls += 1
      recordDisplaySignedUrlBatchSigned(chunk.length)
      let rows = null
      try {
      ...
      for (const path of chunk) {
        ...
        if (url) {
          cache.set(path, url)
          storeDisplaySignedUrl(path, url, expiresIn)
...
export async function signSavedPhotoGridRows(rows, opts = {}) {
  ...
  const phaseCReportPathsInBatch = collected.paths.filter((p) =>
  ...
  const hydrationTiming = getActiveDiaryHydrationTimingSession()
  if (hydrationTiming) {
    hydrationTiming.notePhotoSigningStart({
      photoRows: list.length,
      ...
      reportPathsInGridBatch: phaseCReportPathsInBatch,
    })
  }
  ...
  let thumbSigned = 0
  let legacySigned = 0
  const phaseCReportPathsInGridBatch = phaseCReportPathsInBatch
  ...
  if (hydrationTiming) {
    ...
    let reportFallbackPaths = 0
    for (let i = 0; i < signed.length; i += 1) {
      const row = list[i]
      const thumbPath = String(row.thumbnail_path || row.thumbnailPath || '').trim()
      if (thumbPath && signed[i].preview && !signed[i].thumbnailPreview) {
        reportFallbackPaths += 1
      }
    }
    hydrationTiming.notePhotoSigningComplete(diaryHydrationSigningSummary({
      collected: {
        totalPhotoRows: list.length,
        thumbPathCount: collected.thumbPathCount,
        legacyPathCount: collected.legacyPathCount,
        paths: collected.paths,
        reportPathsInBatch: phaseCReportPathsInGridBatch,
        urlsReturned: thumbSigned + legacySigned,
      },
      stats,
  ...
  if (process.env.NODE_ENV !== 'production') {
    const ended = typeof performance !== 'undefined' && performance.now
      ...
      : Date.now()
    const stats = session.stats?.() || {}
    const displayCache = getDisplaySignedUrlCacheDiag()
    console.info('[zlog:photo-display] Phase D grid hydrate (thumb-first batch)', {
      ...
      displayCache: {
        hits: displayCache.hits,
        misses: displayCache.misses,
        expired: displayCache.expired,
        batchSigned: displayCache.batchSigned,
      },
    })
  }
