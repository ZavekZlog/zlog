...
/** Edit-session quarter turns only. 0 / invalid → no extra rotation. */
export function normalizeEditSessionRotationDegrees(degrees) {
...
/**
 * Apply the user's edit-session rotation onto an already EXIF-upright decode.
 * Full frame is drawn — no crop. 90/270 swap width and height.
 */
function bakeEditSessionRotation(source, sourceWidth, sourceHeight, degrees, documentRef) {
  const extra = normalizeEditSessionRotationDegrees(degrees)
  if (!extra) {
    return { source, width: sourceWidth, height: sourceHeight }
  }
  const swap = extra === 90 || extra === 270
  const outW = swap ? sourceHeight : sourceWidth
  const outH = swap ? sourceWidth : sourceHeight
  const canvas = documentRef.createElement('canvas')
  canvas.width = Math.max(1, outW)
  canvas.height = Math.max(1, outH)
  const ctx = canvas.getContext('2d')
  if (!ctx) {
    throw new ZlogPhotoPipelineError('canvas-unavailable', 'Could not create canvas for photo rotation')
  }
  ctx.translate(outW / 2, outH / 2)
  ctx.rotate((extra * Math.PI) / 180)
  ctx.drawImage(source, -sourceWidth / 2, -sourceHeight / 2)
  return { source: canvas, width: outW, height: outH }
}
...
export async function prepareZlogPhoto(source, options = {}) {
  ...
  try {
    ...
    const rotated = bakeEditSessionRotation(
      img,
      width,
      height,
      options.extraRotationDegrees,
      documentRef,
    )
