 ...
 * Rotation remains metadata; annotations remain overlay/JSON — neither invalidates.
 *
...
/**
 * Ensure a photo has a ready prepared pair — reuse shadowPrepare or regenerate once.
 * @param {object} photo
 * @param {{ prepareFn?: typeof prepareZlogPhoto }} [opts]
 */
export async function ensurePreparedPhotoAssets(photo, opts = {}) {
  if (isPreparedPhotoReadyForPersist(photo)) {
    return {
      ok: true,
      reused: true,
  ...
  try {
    const prepared = await prepareFn(photo.file)
    return {
      ok: true,
      reused: false,
...
export async function uploadPreparedPhotoAssets(supabase, args = {}) {
  ...
  try {
    await putObject(reportPath, reportBlob, reportMeta.mimeType || ZLOG_PHOTO_MIME)
  } catch (err) {
  ...
  if (thumbnailBlob instanceof Blob && thumbnailBlob.size > 0) {
    try {
      await putObject(thumbPath, thumbnailBlob, thumbnailMeta.mimeType || ZLOG_PHOTO_MIME)
