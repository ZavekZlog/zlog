...
/** Field-level Save Area copy — must stay aligned with diary Photo Workspace labels. */
export const FIELD_WORK_AREA_NAME_ERROR = 'Enter a work area name'
...
/** Share flush fallback when commitUnsavedAreaForShare fails without a specific message. */
export const SHARE_UNSAVED_AREA_INCOMPLETE_MESSAGE =
  'Finish this work area (name and photos), or clear it, before Save & Share.'

/**
 * True when a diary page/global error is specifically the missing work-area-name
 * validation from Save & Share — safe to clear when the user fixes Area name.
 * @param {unknown} message
 */
export function isMissingWorkAreaNamePageError(message) {
  const m = String(message || '').trim()
  if (!m) return false
  return m === SHARE_UNSAVED_AREA_NAME_MESSAGE || m === FIELD_WORK_AREA_NAME_ERROR
}

/**
 * True when the page error is the generic unfinished-area Share fallback —
 * not missing-name / missing-photos / missing-layout copy.
 * @param {unknown} message
 */
export function isUnfinishedAreaFallbackPageError(message) {
  return String(message || '').trim() === SHARE_UNSAVED_AREA_INCOMPLETE_MESSAGE
}

/**
 * Keep the unfinished-area fallback only while PhotoWorkspace still has a real
 * unsaved area for Share. Other page errors pass through unchanged.
 *
 * @param {unknown} error
 * @param {boolean} hasUnsavedAreaForShare
 * @returns {string}
 */
export function visibleDiaryUnfinishedAreaPageError(error, hasUnsavedAreaForShare) {
  const message = String(error || '')
  if (!isUnfinishedAreaFallbackPageError(message)) return message
  return hasUnsavedAreaForShare === true ? message : ''
}
