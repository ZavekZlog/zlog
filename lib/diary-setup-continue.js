/**
 * Site Diary setup continue — project link + programme dates + draft create.
 ...
 *
 * @returns {{ ok: true } | { ok: false, message: string, field: 'required'|'dates' }}
 */
 ...
 * Avoids `.select()` of date columns after write — RETURNING can fail under RLS
 * even when the write succeeded, which previously aborted navigation with no
 * visible feedback next to the CTA.
 *
 * @param {object} opts
...
export async function persistSetupProject({ supabase, plan }) {
  ...
  if (plan.mode === 'update') {
    ...
    const { error: updateError } = await supabase
      .from('projects')
      .update({
        start_date: plan.dates.start_date,
        planned_completion_date: plan.dates.planned_completion_date,
      })
      .eq('id', plan.projectId)
    if (updateError) {
      throw new Error(updateError.message || 'We couldn’t save the project dates. Check your connection and try again.')
    }
    return plan.projectId
  }
