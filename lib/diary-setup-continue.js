...
/**
 * Resolve Project Name for continue validation — matches what the user sees when an
 * existing project is selected but the typed name state has not synced (datalist /
 * sticky preload edge cases on mobile).
 *
 * @param {unknown} projectName
 * @param {unknown} selectedProjectId
 * @param {Array<{ id?: string, name?: string }>} existingProjects
 * @param {string} [newProjectSentinel]
 */
export function resolveSetupProjectName(
  projectName,
  selectedProjectId,
  existingProjects = [],
  newProjectSentinel = NEW_PROJECT_SENTINEL,
) {
  const trimmed = String(projectName ?? '').trim()
  if (trimmed) return trimmed
  if (selectedProjectId && selectedProjectId !== newProjectSentinel) {
    const match = (Array.isArray(existingProjects) ? existingProjects : []).find(
...
function coalesceTrimmed(stateValue, domValue) {
...
/**
 * Build the canonical setup form object used by Continue validation and persistence.
 * Prefer React state; fall back to live input values when state has not synced yet.
 *
 * @param {object} state
 * @param {{
 *   existingProjects?: Array<{ id?: string, name?: string }>,
 *   selectedProjectId?: string,
 *   dom?: { projectName?: string, author?: string, reportingOnBehalfOf?: string, reportDate?: string },
 * }} [opts]
 */
export function buildDiarySetupContinueForm(state, {
  existingProjects = [],
  selectedProjectId = NEW_PROJECT_SENTINEL,
  dom = {},
} = {}) {
  const rawProjectName = coalesceTrimmed(state?.projectName, dom.projectName)
  return {
    projectName: resolveSetupProjectName(rawProjectName, selectedProjectId, existingProjects),
    author: coalesceTrimmed(state?.author, dom.author),
    reportingOnBehalfOf: coalesceTrimmed(state?.reportingOnBehalfOf, dom.reportingOnBehalfOf),
    reportDate: coalesceTrimmed(state?.reportDate, dom.reportDate),
    startDate: String(state?.startDate ?? '').trim(),
    plannedCompletionDate: String(state?.plannedCompletionDate ?? '').trim(),
    workingDaysPerWeek: String(state?.workingDaysPerWeek ?? '').trim(),
    authorRole: String(state?.authorRole ?? '').trim(),
    shift: state?.shift ?? '',
    currentPhase: String(state?.currentPhase ?? '').trim(),
    projectAddress: String(state?.projectAddress ?? '').trim(),
    projectManager: String(state?.projectManager ?? '').trim(),
    projectReference: String(state?.projectReference ?? '').trim(),
    brandLogoUrl: state?.brandLogoUrl ?? null,
    brandingId: state?.brandingId ?? null,
    brandColor: state?.brandColor ?? null,
    reportingCompany: String(state?.reportingCompany ?? '').trim(),
  }
}
...
export async function runDiarySetupContinue({
  ...
  return {
    ...
    current_phase: String(form.currentPhase || '').trim() || null,
