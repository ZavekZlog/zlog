...
describe('Share CTA prepares PDF on first tap', () => {
  it('diary Share persists then prepares and shares PDF in the same gesture', () => {
    assert.match(diaryPage, /'Save & Share'/)
    assert.match(diaryPage, /Preparing PDF…/)
    ...
    assert.ok(saveIdx > 0)
    const saveBlock = diaryPage.slice(saveIdx, saveIdx + 18000)
    ...
    assert.doesNotMatch(saveBlock, /pendingSharePdfRef/)
    assert.doesNotMatch(saveBlock, /Tap .* again to open/)
    // Desktop-only download when Web Share API is absent — not after a failed Android share.
