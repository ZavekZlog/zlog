/**
 * Site Diary final Save / Share workflow โ€” completion UI + explicit PDF delivery.
 */
...
describe('Save / Share does not force download on click', () => {
  it('diary Save / Share navigates to complete โ€” does not call download/share on save', () => {
  ...
  it('bakes cover EXIF orientation into upright pixels for PDF embed', () => {
    assert.match(shareLib, /PDF_PHOTO_PIPELINE_ID/)
    ...
    assert.match(shareLib, /coverPhotoUrl = await uprightCoverSrcForPdf\(coverSignedUrl\)/)
  ...
  it('complete page does not download or share on mount', () => {
    ...
    // Status starts empty โ€” no permanent fallback box.
...
describe('completion / share UI hierarchy', () => {
  ...
  it('uses reusable ReportCompleteSharePanel with Save before Share Report', () => {
    ...
    // Share is a section heading โ€” not a large standalone Share CTA.
  ...
  it('does not permanently show a native-sharing fallback on the completion screen', () => {
    assert.doesNotMatch(completePage, /Native sharing isnโ€t available/)
    assert.doesNotMatch(sharePanel, /Native sharing isnโ€t available/)
    assert.doesNotMatch(sharePanel, /Web Share API|navigator\.canShare|browser capacit/i)
    // Status area is conditional only โ€” empty by default.
    assert.match(completePage, /useState\(''\)/)
...
describe('capability helpers + graceful fallback', () => {
  ...
  it('More fallback copy is short and non-technical', () => {
    assert.match(diaryWhatsAppUnavailableMessage(), /canโ€t send the PDF through WhatsApp automatically/i)
    assert.match(diaryEmailFallbackMessage(), /canโ€t attach the PDF to email automatically/i)
    const moreMsg = diaryNativeShareUnavailableMessage()
    assert.match(moreMsg, /More options arenโ€t available/i)
  ...
  it('native share sends the File and treats sheet cancel as success', () => {
    ...
    assert.match(block, /ok: true, aborted: true/)
  })
