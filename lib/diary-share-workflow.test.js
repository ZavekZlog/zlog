...
describe('Share CTA save-then-prepare workflow', () => {
  it('live diary Save persists first, prepares PDF after save, and shares from prepared file on second tap', () => {
    assert.match(diaryPage, /SAVE_CTA_IDLE_LABEL/)
    assert.match(diaryPage, /SAVE_CTA_SAVING_LABEL/)
    assert.match(diaryPage, /tryAcquireSaveOperationLock/)
    assert.match(diaryPage, /shouldIgnoreDuplicateSaveTap/)
    assert.match(diaryPage, /SAVE_CTA_PREPARING_LABEL/)
    ...
    assert.ok(saveIdx > 0)
    const saveBlock = diaryPage.slice(saveIdx, saveIdx + 120000)
    assert.match(saveBlock, /runSiteDiaryPdfExportToShareReadyArtifact/)
    assert.doesNotMatch(saveBlock, /prepareSiteDiaryPdf/)
...
describe('Save PDF lets the user choose filename and folder', () => {
  ...
  it('cancelling the Save As dialog is not an error and not a silent download', () => {
    ...
    assert.match(block.slice(abortIdx, abortIdx + 220), /ok: true, cancelled: true/)
...
describe('capability helpers + graceful fallback', () => {
  ...
  it('live Save & Share starts worker export only after finalize succeeds', () => {
    const saveIdx = diaryPage.indexOf('const handleSave')
    ...
    const workerIdx = saveBlock.indexOf('runSiteDiaryPdfExportToShareReadyArtifact')
    assert.ok(finalizeIdx > 0)
    assert.ok(savedIdIdx > finalizeIdx)
    assert.ok(workerIdx > savedIdIdx)
    const failBeforeWorker = saveBlock.slice(savedIdIdx, workerIdx)
    assert.match(failBeforeWorker, /failSave\(/)
    assert.doesNotMatch(failBeforeWorker, /prepareSiteDiaryPdf/)
    assert.doesNotMatch(saveBlock, /prepareSiteDiaryPdf/)
  })
})
