...
test('diary page session expiry recovers via Sign in to Save (not enabled Save Changes)', () => {
  ...
  assert.match(page, /zlog-save-banner/)
  assert.match(page, /POST_SAVE_SHARE_DELAY_MS/)
  assert.match(page, /saveNavTimerRef\.current = setTimeout/)
  // Saved ✓ must paint before Share handoff; never optimistic before finalizeSiteDiarySave.
  const justSavedIdx = page.indexOf('setJustSaved(true)')
  const shareNavIdx = page.indexOf('router.replace(shareHref)')
  assert.ok(justSavedIdx > 0 && shareNavIdx > justSavedIdx, 'Saved ✓ state precedes Share navigation')
