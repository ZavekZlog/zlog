...
describe('Site Diary — equal choice groups', () => {
  ...
  it('Location Walk after-save offers Add Another and Continue as equal strong neutrals', () => {
    ...
    assert.match(block, /copy\.continueReport/)
    assert.doesNotMatch(block, /PrimaryCTA/)
    assert.equal((block.match(/<EqualChoiceButton\b/g) || []).length, 2)
  })

  it('Location Walk review with areas keeps Add Another / Continue equal (empty create stays primary)', () => {
    ...
    assert.ok(dualStart >= 0, 'dual-choice branch not found')
    const dual = block.slice(dualStart)
    assert.doesNotMatch(dual, /PrimaryCTA/)
    assert.equal((dual.match(/<EqualChoiceButton\b/g) || []).length, 2)
    assert.match(dual, /copy\.addAnother/)
    assert.match(dual, /copy\.continueReport/)
  })

  it('Save Area remains the Location Walk create progression PrimaryCTA', () => {
