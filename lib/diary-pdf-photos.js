...
import { ZLOG_REPORT_MAX_EDGE } from './photo-workspace/image-pipeline.js'
...
/** Work/progress photo JPEG quality at PDF embed only (stored report.jpg unchanged). */
export const PDF_WORK_PHOTO_EMBED_JPEG_QUALITY = 0.85

const PDF_PHOTO_JPEG_QUALITY = PDF_WORK_PHOTO_EMBED_JPEG_QUALITY

/**
 * Layout-aware longest-edge cap for work/progress photos embedded in PDF.
 * Cover uses diary-cover-photo PDF embed constants.
 */
export function pdfWorkPhotoMaxEdgeForLayout(layout) {
  const key = String(layout || 'grid4').toLowerCase()
  if (key === 'full') return 1600
  if (key === 'grid6') return 900
  return 1100
}

/** Future document-evidence rows keep report-grade pixels; not used by Site Diary worker today. */
function pdfWorkPhotoEmbedMaxEdge(photo, layout) {
  const role = String(photo?.pdf_embed_role ?? photo?.pdfEmbedRole ?? '').trim().toLowerCase()
  if (role === 'document_evidence' || role === 'document') {
    return ZLOG_REPORT_MAX_EDGE
  }
  return pdfWorkPhotoMaxEdgeForLayout(layout)
}
...
/** Test isolation — production cache is session-scoped by design. */
export function clearBakedPhotoSrcCacheForTests() {
  bakedPhotoSrcCache.clear()
}
...
export async function buildDiaryPdfPhotos(photos = [], resolveSrc, options = {}) {
  ...
  const prepared = await mapWithConcurrency(list, concurrency, async (photo, i) => {
    ...
    try {
      ...
      const maxEdge = pdfWorkPhotoEmbedMaxEdge(photo, layout)
      let src = null
      let outRotation = rotationDegrees
      let flattenBlob = localBlob

      if (passThrough) {
        bumpShareTimingCountSilent('photoPassThroughCount')
        outRotation = 0
        if (sourceBlob) {
          flattenBlob = sourceBlob
          if (localBlob) rememberPreparedSessionBlob(photo, localBlob)
        } else if (baseSrc && typeof document !== 'undefined' && /^https?:\/\//i.test(String(baseSrc))) {
          const fetchStarted = shareTimingNow()
      ...
      const bakeBaseSrc = passThrough && flattenBlob ? path : baseSrc
      const cacheKey = flattenBlob
        ? null
        : photoBakeCacheKey(bakeBaseSrc, outRotation, maxEdge)
      src = cacheKey ? bakedPhotoSrcCache.get(cacheKey) || null : null
      if (!src) {
        if (typeof document !== 'undefined') {
          if (bakeBaseSrc || flattenBlob) {
            src = await flattenPhotoSrcForPdf(bakeBaseSrc, maxEdge, flattenBlob)
            ...
            if (outRotation) {
              ...
              try {
                src = await applyRotationToImageSrc(src, outRotation)
              } catch {
                src = flattenedSrc
              }
        ...
        } else if (typeof options.serverImageFlatten === 'function') {
          src = await options.serverImageFlatten({
            baseSrc: bakeBaseSrc,
            maxEdge,
            localBlob: flattenBlob,
            rotationDegrees: outRotation,
            photo,
            path,
            jpegQuality: PDF_PHOTO_JPEG_QUALITY,
          })
          bumpShareTimingCountSilent('photoFetchBakeCount')
          bumpShareTimingCountSilent('photoPrepareCount')
          bumpShareTimingCountSilent('photoBakeCount')
