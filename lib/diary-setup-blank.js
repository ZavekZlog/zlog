...
import { DEFAULT_NEW_PROJECT_WORKING_DAYS } from './project-sticky-fields.js'

/**
 * Explicit clean initial-state factory for a brand-new diary setup form.
 * Permitted defaults: Author profile, Reporting On Behalf Of, Report Date
 * (today), and optional company-profile branding.
 *
 * @param {{
 *   authorName?: string,
 *   authorRole?: string,
...
export function blankDiarySetupFormState({
  ...
  return {
    ...
    reportingOnBehalfOf: String(reportingOnBehalfOf || '').trim(),
...
/**
 * Session draft restore is only for Project & Report Details (has report id).
 ...
 * clears every project-level field immediately. Keeps user/report defaults.
 *
 * @param {object} [state]
 */
...
export function isCleanScratchSetupState(state, expected = {}) {
  ...
  if (
    ...
    && state.workingDaysPerWeek !== DEFAULT_NEW_PROJECT_WORKING_DAYS
  ) return false
  ...
  if (
    expected.reportingOnBehalfOf != null
    && state.reportingOnBehalfOf !== expected.reportingOnBehalfOf
  ) return false
