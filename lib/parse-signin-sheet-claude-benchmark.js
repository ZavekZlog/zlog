...
/** Per-column header label tokens (semantic indicators, not a flat cell bag). */
const HEADER_DATE_TOKENS = new Set(['date'])
const HEADER_NAME_TOKENS = new Set([
  'name',
  'person',
  'person name',
  'operative',
...
const HEADER_COMPANY_TOKENS = new Set(['company', 'subcontractor', 'employer'])
const HEADER_TRADE_TOKENS = new Set(['trade', 'role'])
const HEADER_TIME_IN_TOKENS = new Set(['time in', 'timein', 'time-in', 'sign in'])
const HEADER_TIME_OUT_TOKENS = new Set(['time out', 'timeout', 'time-out', 'sign out'])
const HEADER_SIGNATURE_TOKENS = new Set(['signature', 'sign'])

/**
 * Minimum distinct column-header indicators required to reject a row as the
 * table heading. One accidental header-word cell must not drop a real worker.
 */
const MIN_STRUCTURAL_HEADER_INDICATORS = 3
 ...
 * @returns {string}
 */
function firstFieldKey(row, keys) {
  for (const key of keys) {
    const value = fieldKey(row[key])
...
/**
 * Count distinct columns whose value is a known heading label for that column.
 * Blank cells do not count. Corrupted non-label values (e.g. Trade → "Painter")
 * do not count for that column — other intact heading labels still can.
 *
 * @param {Record<string, unknown>} row
 * @returns {number}
 */
function countStructuralHeaderIndicators(row) {
  /** @type {{ keys: string[], tokens: Set<string> }[]} */
  const columns = [
    { keys: ['date', 'work_date', 'day'], tokens: HEADER_DATE_TOKENS },
    { keys: ['person_name', 'name', 'operative'], tokens: HEADER_NAME_TOKENS },
    {
      keys: ['company', 'subcontractor', 'employer'],
      tokens: HEADER_COMPANY_TOKENS,
    },
    { keys: ['trade', 'role'], tokens: HEADER_TRADE_TOKENS },
    {
      keys: ['time_in', 'signed_in_at', 'signin_at'],
      tokens: HEADER_TIME_IN_TOKENS,
    },
    {
      keys: ['time_out', 'signed_out_at', 'signout_at'],
      tokens: HEADER_TIME_OUT_TOKENS,
    },
    { keys: ['signature', 'sign', 'signed'], tokens: HEADER_SIGNATURE_TOKENS },
  ]

  let indicators = 0
  for (const column of columns) {
    const value = firstFieldKey(row, column.keys)
    if (value && column.tokens.has(value)) indicators += 1
  }
  return indicators
}
 ...
 *
 * Structural rule: MULTIPLE (≥3) distinct column-header indicators ⇒ header,
 * even when Claude corrupts one heading cell into a plausible value (e.g. Trade
 * → "Painter") or leaves one/two heading cells blank.
 *
 * A single accidental header-word cell does NOT reject a genuine worker.
 * Empty / illegible names are NOT treated as header labels by themselves.
 *
...
export function isClaudeBenchmarkHeaderRow(row) {
  ...
  const structuralIndicators = countStructuralHeaderIndicators(row)
  if (structuralIndicators >= MIN_STRUCTURAL_HEADER_INDICATORS) return true

  // Legacy clean-header path: every non-empty cell is a known column label.
  const candidates = [
    row.date,
    row.work_date,
    row.day,
  ...
  if (headerHits.length === candidates.length && headerHits.length >= 2) return true
...
/**
 * Trade-only consensus merge. When first-pass trade is blank, accepts consensus
 * normalized_value only when consensus explicitly clears review (no app-side invention).
 *
 * @param {string|null} rawValue
 * @param {unknown} review
 */
export function resolveClaudeBenchmarkTradeReviewField(rawValue, review) {
  const raw = nullableIdentityText(rawValue)
  if (raw != null) {
    return resolveClaudeBenchmarkIdentityReviewField(rawValue, review)
  }
  ...
  if (review && typeof review === 'object') {
    ...
    const normalized = nullableIdentityText(review.normalized_value)
    const confidence = nullableConfidence(review.confidence)
    if (!needsReview && normalized != null) {
      return {
        raw_value: null,
        reviewed_value: normalized,
        needs_review: false,
        confidence,
      }
    }
...
/**
 * Deterministic construction-trade alias → canonical trade.
 * Uses the shared UK construction vocabulary (Spark→Electrician, Scaff→Scaffolder, …).
 * No fuzzy nearest-trade matching. No Scaff→Electrician rule.
 *
 * @param {unknown} trade
 * @returns {string|null}
 */
export function normalizeClaudeBenchmarkTradeValue(trade) {
  return canonicalizeConstructionTrade(trade)
}
