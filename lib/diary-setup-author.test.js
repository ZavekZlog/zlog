...
describe('diary setup — Report Author order', () => {
  it('Author follows Reporting Company and Reporting On Behalf Of; Project Details comes first', () => {
    ...
    const coverIdx = setupPage.indexOf('title="Cover photo"')
    assert.ok(projectIdx > 0 && projectIdx < companyIdx, 'Project Details first')
    ...
    assert.ok(coverIdx > roleIdx, 'Cover photo after Author')
    assert.ok(stickyIdx > projectIdx && datesIdx > stickyIdx && shiftIdx > datesIdx)
    assert.ok(shiftIdx < companyIdx, 'Shift stays inside Project Details before Reporting Company')
    assert.ok(stickyIdx < companyIdx, 'sticky fields stay inside Project Details before Reporting Company')
  })
  ...
  it('Author Role appears directly beneath Author Name as free-text input', () => {
    ...
    const roleBlock = setupPage.slice(
      ...
      setupPage.indexOf('title="Cover photo"'),
    )
