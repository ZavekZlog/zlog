...
/** Resize + JPEG-compress for vision (max edge 1600px). Browser-display pixels once — no second EXIF bake. */
export async function fileToVisionDataUrl(file, maxEdge = 1600, quality = 0.82) {
  const { orientedImageToDataUrlForPdf } = await import('@/lib/image-orientation')
  const result = await orientedImageToDataUrlForPdf(file, maxEdge, quality)
  if (typeof process !== 'undefined' && process.env.NODE_ENV === 'development') {
    ...
    console.info('[sign-in OCR] image orientation', {
      ...
      bakedManualOrientation: result.bakedManualOrientation,
      width: result.width,
