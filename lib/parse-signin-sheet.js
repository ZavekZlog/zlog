...
export function normalizeOcrSignInRows(rawRows = []) {
  ...
  return list
    .map((row) => {
      ...
      const sourceRow = Number(row.source_row ?? row.sourceRow ?? row.physical_row)
      return {
        ...
        source_row: Number.isFinite(sourceRow) ? sourceRow : null,
      }
    })
    .filter(Boolean)
}
...
export function labourFromOcrSheet(rawRows, reportDate, { groupBy = 'trade_company', makeKey, visibleAttendeeCount } = {}) {
  ...
  if (claimed != null && claimed !== normalized.length) {
    warnings.push(
      `Register appears to show ${claimed} attendee${claimed === 1 ? '' : 's'} but OCR returned ${normalized.length}. Check for missing or duplicated rows.`,
    )
  }
...
export const SIGNIN_UPRIGHT_IMAGE_PIPELINE = 'explicit-exif-upright-signin-v2'

/**
 * EXIF-flattened upright JPEG for sign-in preview, persisted evidence, and vision OCR.
 * Raw decode + manual orientation when needed (Galaxy S10 does not reliably browser-orient).
 * No crop; aspect preserved; max edge capped.
 */
export async function fileToVisionPreparedImage(file, maxEdge = 1600, quality = 0.82) {
  const { prepareSignInImageToDataUrl } = await import('./sign-in-image-preparation.js')
  const result = await prepareSignInImageToDataUrl(file, maxEdge, quality)
  const uprightMax = Math.max(result.width, result.height)
  return {
    ...
    imageMeta: {
      ...
      resized: uprightMax > maxEdge,
      reencoded: true,
      pipeline: SIGNIN_UPRIGHT_IMAGE_PIPELINE,
    },
  }
}

/** Resize + JPEG-compress for vision (max edge 1600px). Single explicit EXIF flatten — shared with preview/persist. */
export async function fileToVisionDataUrl(file, maxEdge = 1600, quality = 0.82) {
  const prepared = await fileToVisionPreparedImage(file, maxEdge, quality)
