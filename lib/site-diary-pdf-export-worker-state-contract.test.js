...
describe('SITE_DIARY_PDF_EXPORT_JOB worker state (2C-2A)', () => {
  ...
  it('X — 2C-2A migration does not deploy worker, bucket, or PDF runtime', () => {
    ...
    assert.doesNotMatch(sql, /extend_site_diary_pdf_export/)
    assert.doesNotMatch(sql, /heartbeat/)
    const workerShell = join(root, 'worker/site-diary-pdf-export/run.mjs')
    try {
      readFileSync(workerShell, 'utf8')
    } catch {
      assert.fail('worker/site-diary-pdf-export/ is allowed after 2C-2B')
    }
  })
