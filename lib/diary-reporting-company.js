...
export function resolveReportingCompanyIdentity({
  ...
  if (linked?.id) {
    return {
      ...
      brandColor: report?.brand_color || linked.brand_color || null,
    }
  }
...
export function stickyReportingCompanyFromLatest({
  ...
  return resolveReportingCompanyIdentity({ defaultProfile })
}
...
export async function fetchStickyReportingCompany(supabase) {
  ...
  const { data: latestReport } = await supabase
    .from('daily_reports')
    .select('branding_id, brand_logo_url, brand_color, created_at')
    .order('created_at', { ascending: false })
    .limit(1)
    .maybeSingle()
  ...
  return stickyReportingCompanyFromLatest({
    latestReport,
    latestBrandingRow,
    defaultProfile,
  })
...
/**
 * Load Reporting Company for Edit Report Details of an existing diary.
 */
