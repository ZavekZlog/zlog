 ...
 * @param {string|null} [opts.brandingId]
 * @param {string|null} [opts.brandColor]
 * @param {string|null} [opts.userId] Same-click verified owner id. When non-empty,
 *   skips auth.getUser(); callers that omit it keep the existing getUser gate.
 */
export async function persistReportingCompanyIdentity(supabase, {
  ...
  let ownerUserId = suppliedUserId
  if (!ownerUserId) {
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) {
 ...
 * Pass `{ user }` (including null) to reuse an already-resolved identity and
 * skip a second auth.getUser(). Omit it to keep the existing getUser gate.
 */
export async function fetchReportingCompanyForReport(supabase, report, opts = {}) {
  const brandingPromise = report?.branding_id
    ? supabase
      .from('company_brandings')
      .select('id, company_name, logo_url, brand_color')
  ...
  let resolvedUser
  if (Object.prototype.hasOwnProperty.call(opts, 'user')) {
    resolvedUser = opts.user || null
  } else {
    const { data: { user } } = await supabase.auth.getUser()
    resolvedUser = user || null
  }

  const defaultPromise = resolvedUser?.id
    ? supabase
      .from('company_brandings')
      .select('id, company_name, logo_url, brand_color, is_default')
  ...
  const [{ data: brandingRow }, { data: defaultProfile }] = await Promise.all([
    brandingPromise,
    defaultPromise,
  ])
