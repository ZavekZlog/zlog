 ...
 * Today's existing diary opens on the workbench (inline Project Details; optional ?details=open).
...
/**
 * Existing-diary workbench URL with inline Project Details expanded (no mode change).
 *
 * @param {string} projectId
 * @param {string} reportId
 ...
 * @returns {string | null}
 */
export function workbenchInlineProjectDetailsHref(projectId, reportId, { edit = true } = {}) {
  if (!projectId || !reportId) return null
  const base = edit ? diaryEditHref(projectId, reportId) : diaryViewHref(projectId, reportId)
  if (!base) return null
  return `${base}&details=open`
}
...
export function openExistingDiaryHref({
  ...
  if (isTodaysDiary(reportDate, today)) {
    if (detailsOpen) {
      return workbenchInlineProjectDetailsHref(projectId, reportId, { edit: true })
    }
    return diaryEditHref(projectId, reportId)
  }
