...
import { resolveCoverPdfSource } from '@/lib/diary-cover-photo'
...
export async function prepareSiteDiaryPdf({
  ...
  try {
    ...
    const pdfAssetPromise = signPdfReportAssets(
      supabase,
      report,
      (signedCoverUrl) => resolveCoverPdfSource(signedCoverUrl, {
    ...
    let photoRows = []
    {
      const primary = await supabase
        ...
        .order('sequence')
      if (primary.error && /processing_version/i.test(primary.error.message || '')) {
