...
import { canSkipPreparedCoverSign } from '@/lib/diary-cover-prepared-session-cache'
 ...
 */
async function flattenCoverBlobForPdf(
  blob,
  maxEdge = PDF_COVER_EMBED_MAX_EDGE,
  quality = PDF_COVER_EMBED_JPEG_QUALITY,
) {
...
export async function uprightCoverSrcForPdf(url) {
  ...
  const baked = await flattenCoverBlobForPdf(blob)
  const { dataUrl } = baked
...
export async function prepareSiteDiaryPdf({
  ...
  try {
    ...
    const skipCoverSign = canSkipPreparedCoverSign({
    ...
    patchShareTimingCounts({ coverSignSkipped: skipCoverSign })
    const pdfAssetPromise = signPdfReportAssets(
      ...
      async (signedCoverUrl) => {
        ...
        return resolveCoverPdfSource(signedCoverUrl, {
          ...
          embedPreparedCoverBlob: async (coverBlob) => {
            const baked = await flattenCoverBlobForPdf(coverBlob)
    ...
    // Background idle prepares skip this to avoid rewriting IndexedDB after every quiet period.
    if (!skipShareCacheWrite) {
      try {
