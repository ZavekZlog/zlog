...
describe('prepared work-photo session Blob cache', () => {
  ...
  it('in-flight join is one fetch per key and failed fetch does not poison retry', async () => {
    const key = preparedWorkPhotoSessionCacheKey(makePrepared())
    let starts = 0
    let release
    const hung = new Promise((resolve) => {
      release = resolve
    })
    const first = joinPreparedWorkPhotoSessionFetch(key, async () => {
      starts += 1
      await hung
      return jpegBlob()
    })
    const second = joinPreparedWorkPhotoSessionFetch(key, async () => {
      starts += 1
      return jpegBlob('OTHER')
    })
    assert.ok(peekPreparedWorkPhotoSessionInflight(key))
    release()
    const [a, b] = await Promise.all([first, second])
    assert.equal(starts, 1)
    assert.equal(a.size, b.size)
    assert.equal(peekPreparedWorkPhotoSessionInflight(key), null)

    await assert.rejects(() =>
      joinPreparedWorkPhotoSessionFetch(key, async () => {
        throw new Error('boom')
      }),
    )
    assert.equal(peekPreparedWorkPhotoSessionInflight(key), null)
    const retry = await joinPreparedWorkPhotoSessionFetch(key, async () => jpegBlob())
    assert.equal(retry.size, BYTE_SIZE)
  })
})
