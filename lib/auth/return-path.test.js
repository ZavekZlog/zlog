...
describe('loginUrlWithReturn', () => {
  it('embeds recovery=1 and a safe next param', () => {
    const path = '/dashboard/project/abc/diary?report=rep-1'
    assert.equal(
      loginUrlWithReturn(path),
      `/login?recovery=1&next=${encodeURIComponent(path)}`,
    )
  })
...
describe('resolvePostLoginDestination', () => {
  it('returns dashboard after voluntary sign-out even when next is present', () => {
  ...
  it('preserves protected workbench return only with recovery=1', () => {
    const path = '/dashboard/project/abc/diary?report=rep-1'
    assert.equal(resolvePostLoginDestination({ next: path, recovery: '1' }), path)
    assert.equal(resolvePostLoginDestination({ next: path, signedOut: false }), '/dashboard')
  })
})

describe('isStaleVoluntaryLoginNext', () => {
  it('flags diary setup shim URLs', () => {
    assert.equal(
      isStaleVoluntaryLoginNext('/dashboard/diary/setup?report=r&project=p'),
      true,
    )
    assert.equal(isStaleVoluntaryLoginNext('/dashboard/project/x/diary'), false)
  })
})
