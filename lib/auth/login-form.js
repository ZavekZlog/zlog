...
/**
 * Mark that the user explicitly intends to sign in (Sign In click or Enter).
 * Autofill filling fields must not set this.
 */
export function markLoginSubmitIntent(intentRef) {
...
/**
 * Consume a one-shot explicit login intent.
 * Returns true only when Sign In / Enter previously marked intent.
 */
export function consumeLoginSubmitIntent(intentRef) {
  if (!intentRef || typeof intentRef !== 'object' || intentRef.current !== true) {
    return false
  }
