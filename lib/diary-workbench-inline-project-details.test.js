/**
 * Unit 2 — inline Project Details on Site Diary workbench (lazy-loaded editor).
 */
...
describe('Unit 2 — workbench mount', () => {
  ...
  it('inline persist uses controller without router.push on workbench', () => {
    assert.match(editorSrc, /persistProjectDetails/)
    assert.match(editorSrc, /hostMode:\s*'workbench'/)
  ...
  it('expand/collapse does not bump formReloadToken', () => {
    const block = diaryPage.slice(
      diaryPage.indexOf('setProjectDetailsExpanded'),
