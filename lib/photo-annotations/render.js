...
import { normalizeAnnotationDoc, strokeWidthPx } from './model.js'
...
function shapeHits(shape, nx, ny, pad) {
  if (!shape) return false
  if (shape.type === 'arrow') {
    return distToSegment(nx, ny, shape.x1, shape.y1, shape.x2, shape.y2) <= pad
  }
  if (shape.type === 'freehand') {
    const pts = shape.points || []
    if (!pts.length) return false
    if (pts.length === 1) {
      return Math.hypot(nx - pts[0].x, ny - pts[0].y) <= pad
    }
    for (let i = 1; i < pts.length; i += 1) {
      if (distToSegment(nx, ny, pts[i - 1].x, pts[i - 1].y, pts[i].x, pts[i].y) <= pad) {
        return true
      }
    }
    return false
  ...
  if (shape.type === 'ellipse') {
    const rx = Math.max(0.001, Number(shape.rx) || 0) + pad
    const ry = Math.max(0.001, Number(shape.ry) || 0) + pad
...
export function drawShape(ctx, shape, imageWidth, imageHeight, { selected = false } = {}) {
  ...
  if (selected) {
    // Subtle orange selection outline (mobile-readable, not a redesign).
    ctx.setLineDash([5, 4])
    ctx.strokeStyle = 'rgba(255, 80, 0, 0.9)'
    ctx.lineWidth = Math.max(1.5, Math.min(2.5, sw * 0.55))
    const bounds = shapeBounds(shape)
    if (bounds) {
      const padPx = Math.max(4, Math.min(w, h) * 0.008)
      ctx.strokeRect(
        toPx(bounds.x, w) - padPx,
        toPx(bounds.y, h) - padPx,
        toPx(bounds.w, w) + padPx * 2,
        toPx(bounds.h, h) + padPx * 2,
      )
    }
  }
...
/** Hit-test normalized point against shapes (top-most first). Touch-friendly pad ~24px. */
export function hitTestShape(doc, nx, ny, imageWidth, imageHeight) {
  const normalized = normalizeAnnotationDoc(doc, imageWidth, imageHeight)
  const minEdge = Math.max(1, Math.min(Number(imageWidth) || 1, Number(imageHeight) || 1))
  const pad = Math.max(0.02, 24 / minEdge)
  for (let i = normalized.shapes.length - 1; i >= 0; i -= 1) {
    const shape = normalized.shapes[i]
    if (shapeHits(shape, nx, ny, pad)) return shape
  }
  return null
}
