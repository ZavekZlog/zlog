  ...
  for (const group of locationWalk) {
    ...
    for (const photo of group.photos || []) {
      ...
      rows.push({
        ...
        // Phase C/D: transient prepare + durable prepared-asset metadata (passthrough).
        shadowPrepare: photo.shadowPrepare || null,
        thumbnailPath: photo.thumbnailPath || null,
        thumbnailPreview: photo.thumbnailPreview || null,
...
export function groupPhotosByArea(flatPhotos = []) {
  ...
  for (const p of flatPhotos) {
    ...
    group.photos.push({
      ...
      thumbnailPreview: p.thumbnailPreview || null,
