...
describe('saved diary viewer — read-only artifact', () => {
  ...
  it('Edit This Diary keeps today’s approved details pre-flight', () => {
    assert.equal(
      editExistingDiaryHref({
        projectId: 'proj-abc',
        reportId: 'rep-today',
...
describe('routing regression — existing diary entry points', () => {
  ...
  it('diary hub opens a saved entry in the read-only viewer', () => {
    ...
    const review = hub.slice(
      ...
      hub.indexOf('const requestUsePreviousForToday'),
    )
  ...
  it('Use a Previous Diary opens Project & Report Details, not the workbench', () => {
    const hub = readFileSync(join(root, 'app/dashboard/diary/page.jsx'), 'utf8')
    const useForToday = hub.slice(
      hub.indexOf('const confirmUsePreviousForToday'),
      hub.indexOf('const startNewReport'),
    )
    // Today's diary is created from the chosen one, then reviewed before the workbench.
    const created = useForToday.indexOf('createTodaysDiaryDraft')
    const details = useForToday.indexOf('projectAndReportDetailsHref')
    assert.ok(created > 0, 'Use for today still creates today’s diary')
    assert.ok(details > created, 'details screen must follow draft creation')
    assert.doesNotMatch(useForToday, /diaryFormHref|compose=1/)
