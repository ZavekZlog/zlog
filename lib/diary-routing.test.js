...
import { readSetupPageSource } from './diary-setup-ui-source.js'
...
describe('today existing diary — workbench inline entry', () => {
  it('opens today’s existing report on the workbench in explicit edit mode', () => {
    assert.equal(
      openExistingDiaryHref({
        projectId: 'proj-abc',
        reportId: 'rep-today',
...
describe('saved diary viewer — read-only artifact', () => {
  ...
  it('the viewer route file exists', () => {
    assert.ok(
      existsSync(join(root, 'components/site-diary/SavedDiaryViewerSurface.jsx')),
      'expected components/site-diary/SavedDiaryViewerSurface.jsx',
    )
  })

  it('DIARY-SAVED-BACK — Zlog Back replaces onto the Saved Diaries list', () => {
    ...
    assert.notEqual(savedReportListHref(), '/dashboard')

    const shells = viewer.match(/<PremiumShell[\s\S]*?>/g) || []
    assert.equal(
      shells.length,
      4,
      'loading, error, review, and Suspense fallback shells',
    )
    for (const shell of shells) {
      assert.match(shell, /backHref=\{savedReportListHref\(\)\}/)
      assert.match(shell, /onBack=\{backToSavedDiaries\}/)
      assert.doesNotMatch(shell, /backHref="\/dashboard"/)
      assert.doesNotMatch(shell, /backHref=\{['"]\/dashboard['"]\}/)
    }

    const handlers = viewer.match(
      /const backToSavedDiaries = \(event\) => \{[\s\S]*?\n  \}/g,
    ) || []
    assert.equal(handlers.length, 2, 'viewer and Suspense-fallback handlers')
    ...
    assert.doesNotMatch(viewer, /backHref="\/dashboard"/)
    assert.doesNotMatch(viewer, /backHref=\{['"]\/dashboard['"]\}/)
...
describe('routing regression — existing diary entry points', () => {
  it('project diary page route exists at the supported path', () => {
    assert.ok(
      existsSync(join(root, 'components/site-diary/SiteDiaryWorkbenchSurface.jsx')),
      'expected components/site-diary/SiteDiaryWorkbenchSurface.jsx',
    )
  })
  ...
  it('View Saved Diaries pushes the list so the 2-card hub remains in history', () => {
    const hub = readFileSync(join(root, 'app/dashboard/diary/page.jsx'), 'utf8')
    const openSaved = hub.slice(
      hub.indexOf('const openSavedDiaries'),
      hub.indexOf('const startNewReport'),
    )
    assert.match(openSaved, /router\.push\(savedReportListHref\(\)\)/)
    assert.doesNotMatch(openSaved, /router\.replace\(savedReportListHref\(\)\)/)
    const leave = hub.slice(
      hub.indexOf('const leaveSavedList'),
      hub.indexOf('const enterSelectionMode'),
    )
  ...
  it('saved diary row keeps one activation path and the unchanged viewer href', () => {
    ...
    assert.match(open, /shouldIgnoreSavedDiaryRowOpen/)
  ...
  it('project-param cleanup cannot override an already-started diary open', () => {
    ...
    assert.ok(
      ...
      'open must arm the guard before navigating to the viewer',
    )
  })
...
describe('workbench Back → Site Diary hub', () => {
  const diaryPage = readFileSync(
    join(root, 'components/site-diary/SiteDiaryWorkbenchSurface.jsx'),
    'utf8',
  )

  it('setup Project & Report Details URL remains for bootstrap flows', () => {
  ...
  it('workbench PremiumShell Back keeps the diaryHubHref target and persistence-aware handler', () => {
    const start = diaryPage.indexOf('const workbenchBackHref')
    assert.ok(start > 0, 'workbenchBackHref must be defined')
    const handlerStart = diaryPage.indexOf('const handleWorkbenchBack', start)
    const handlerEnd = diaryPage.indexOf('const autosaveStatusCopy', handlerStart)
    const handler = diaryPage.slice(handlerStart, handlerEnd)
    const hrefUse = diaryPage.indexOf('backHref={workbenchBackHref}', start)
    assert.ok(hrefUse > start, 'PremiumShell must use workbenchBackHref')
    const onBackUse = diaryPage.indexOf('onBack={handleWorkbenchBack}', hrefUse)
    assert.ok(onBackUse > hrefUse, 'PremiumShell must use the persistence-aware Back handler')
    const block = diaryPage.slice(start, onBackUse + 'onBack={handleWorkbenchBack}'.length)
    ...
    assert.match(block, /onBack=\{handleWorkbenchBack\}/)
    assert.match(handler, /router\.push\(workbenchBackHref\)/)
    assert.doesNotMatch(block, /backHref=\{`\/dashboard\/project\/\$\{projectId\}\/diary`\}/)
  })

  it('Back destination does not create a diary', () => {
    const start = diaryPage.indexOf('const workbenchBackHref')
    const hrefUse = diaryPage.indexOf('backHref={workbenchBackHref}', start)
    const block = diaryPage.slice(start, hrefUse + 'backHref={workbenchBackHref}'.length)
...
describe('Project Details Back → Site Diary hub (no workbench hop)', () => {
  ...
  it('setupBackHref uses diaryHubHref when project context exists', () => {
    const backAt = setupPage.indexOf('const setupBackHref = useMemo')
    const controllerAt = setupPage.indexOf('} = useSiteDiaryProjectDetailsController({', backAt)
    const block = setupPage.slice(backAt, controllerAt)
  ...
  it('PremiumShell and footer Back share the saving-guarded setupBackHref (no onBack / workbench push)', () => {
    assert.match(setupPage, /const setupBackNavigationHref = saving \? null : setupBackHref/)
    assert.match(setupPage, /backHref=\{setupBackNavigationHref\}/)
    assert.match(setupPage, /href=\{setupBackNavigationHref\}/)
