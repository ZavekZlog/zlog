...
export const DEFAULT_NEW_PROJECT_WORKING_DAYS = '5'
...
/**
 * A blank Site Diary setup field must not erase valid project-owned data.
 * Explicit clearing remains available from the project's own details editor.
 *
 * @param {ReturnType<typeof stickyWritePayload>} payload
 * @param {Record<string, unknown>|null|undefined} project
 */
export function preserveSavedStickyFields(payload, project) {
  if (!project) return payload
  const saved = stickyWritePayload(hydrateStickyFromRow(project))
  const next = {
    ...payload,
    site_address: payload.site_address || saved.site_address,
    client_pm: payload.client_pm || saved.client_pm,
    working_days_per_week:
      payload.working_days_per_week == null
        ? saved.working_days_per_week
        : payload.working_days_per_week,
  }
  if (Object.prototype.hasOwnProperty.call(payload, 'project_reference')) {
    next.project_reference = payload.project_reference || saved.project_reference
  }
  return next
}
...
export function stickyProjectSelectColumns() {
  return 'site_address, client_pm, working_days_per_week, project_reference'
}
