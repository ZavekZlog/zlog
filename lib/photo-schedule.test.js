...
describe('photoTileAssignedTo', () => {
  it('returns trimmed value and blank when empty', () => {
    assert.equal(photoTileAssignedTo({ assignedTo: 'Roofing Contractor' }), 'Roofing Contractor')
    assert.equal(photoTileAssignedTo({ assigned_to: '  Site Team  ' }), 'Site Team')
    assert.equal(photoTileAssignedTo({ assignedTo: '' }), '')
    assert.equal(photoTileAssignedTo({ assignedTo: '   ' }), '')
  })

  it('PDF line omits label when blank; includes value when present', () => {
    assert.equal(photoTileAssignedToLine({ assignedTo: '' }), '')
    assert.equal(
      photoTileAssignedToLine({ assignedTo: 'Roofing Contractor' }),
