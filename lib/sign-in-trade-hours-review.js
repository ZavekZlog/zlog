 ...
 */
export function formatSignInOperativeLabourLine(operative, tradeLabel = '') {
  ...
  const tradeSuffix = trade ? ` · ${trade}` : ''
  return `${rowLabel} · ${timeIn}–${timeOut} · ${hoursLabel}${tradeSuffix}`
}

export const SIGNIN_LABOUR_EXCLUSION_HOURS_CONFLICT_MESSAGE =
  'Hours on this trade were edited manually and no longer match the sign-in rows. Adjust hours yourself, or reset them to match the sheet, then exclude someone again.'

export const SIGNIN_LABOUR_EXCLUSION_WORKERS_CONFLICT_MESSAGE =
  'Workers on this trade were edited manually and no longer match the sign-in rows. Adjust workers yourself, or reset them to match the sheet, then exclude someone again.'
...
export function formatVisitorLineFromSignInOperative(operative, tradeLabel = '') {
  ...
  const timeOut = operative.time_out ? String(operative.time_out).trim() : '—'
  const sourceRow = Number.isFinite(Number(operative.source_row)) ? Number(operative.source_row) : null
  const rowSuffix = sourceRow != null ? `Sign-in row ${sourceRow}` : 'Sign-in row —'
...
export function applyOperativeMoveToVisitorsFromReview({
  ...
  const visitorsText = appendVisitorNarrative(existingVisitorsText, visitorLine)

  return {
    ok: true,
    operatives: nextOperatives,
    reviewRows: pruneSignInTradeHoursReviewPlaceholders(nextRows),
    visitorsText,
    visitorLine,
  }
}
