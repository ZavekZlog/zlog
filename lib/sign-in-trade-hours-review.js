...
/**
 * Manual “+ Add trade” rows (no scan rowIds) may be removed from review.
 * Scan-derived buckets must use ⋯ people actions, not row delete.
 * @param {object|null|undefined} row
 */
export function signInTradeHoursReviewRowIsManualOnly(row) {
  return !scanDerivedSignInTradeHoursReviewRow(row)
}
 ...
 * @param {string} rowKey
 */
export function removeManualSignInTradeHoursRow(rows = [], rowKey) {
  const list = Array.isArray(rows) ? rows : []
  const row = list.find((r) => r && r.key === rowKey)
  if (!row || !signInTradeHoursReviewRowIsManualOnly(row)) {
    return list.map((r) => ({ ...r }))
  }
  return removeSignInTradeHoursRow(list, rowKey)
}

/**
 * @param {SignInTradeHoursReviewRow[]} rows
 */
/** Max pointer movement (CSS px) for touch/pen Add trade to count as a tap. */
export const SIGN_IN_ADD_TRADE_TAP_MOVE_THRESHOLD_PX = 10

/**
 * Touch/pen use pointerup + movement threshold (not pointerdown) for Add trade.
 * @param {unknown} pointerType
 * @param {boolean} [disabled]
 */
export function signInAddTradeUsesTapUpActivation(pointerType, disabled = false) {
  if (disabled) return false
  const type = String(pointerType || '')
  return type === 'touch' || type === 'pen'
}

/** @deprecated Use signInAddTradeUsesTapUpActivation — name kept for older imports. */
export function signInAddTradeUsesPointerDownActivation(pointerType, disabled = false) {
  return signInAddTradeUsesTapUpActivation(pointerType, disabled)
}

/**
 * Add trade never runs on pointerdown alone (touch scroll must not create a row).
 */
export function signInAddTradeAddsRowOnPointerDown() {
  return false
}
 ...
 */
export function signInAddTradeTapMovementWithinThreshold(dx, dy, thresholdPx = SIGN_IN_ADD_TRADE_TAP_MOVE_THRESHOLD_PX) {
  const limit = Number(thresholdPx)
  const t = Number.isFinite(limit) && limit > 0 ? limit : SIGN_IN_ADD_TRADE_TAP_MOVE_THRESHOLD_PX
  return Math.hypot(Number(dx) || 0, Number(dy) || 0) <= t
}

/**
 * Peak pointer movement during a touch gesture (scroll can exceed threshold before release).
 * @param {number} startX
 * @param {number} startY
 * @param {number} x
 * @param {number} y
 ...
 */
export function signInAddTradePeakMovementPx(startX, startY, x, y, currentMax = 0) {
  const peak = Math.hypot(Number(x) - Number(startX), Number(y) - Number(startY))
  const prev = Number(currentMax) || 0
 ...
 * }} params
 */
export function signInAddTradeShouldActivateAfterPointerUp(params) {
  const {
    startX,
    startY,
    endX,
    ...
    cancelled = false,
    maxMovementPx = 0,
    thresholdPx = SIGN_IN_ADD_TRADE_TAP_MOVE_THRESHOLD_PX,
  } = params || {}
  if (cancelled) return false
  const peak = Number(maxMovementPx) || 0
  if (!signInAddTradeTapMovementWithinThreshold(peak, 0, thresholdPx)) return false
  const dx = Number(endX) - Number(startX)
  const dy = Number(endY) - Number(startY)
  return signInAddTradeTapMovementWithinThreshold(dx, dy, thresholdPx)
}

/**
 * Pure simulation of SignInOperativeReview Add trade touch → click (for regression tests).
 * @param {{
 *   pointerTypeDown: string,
 ...
 *   thresholdPx?: number,
 * }} scenario
 * @returns {{ rowsAdded: number, clickWouldAdd: boolean }}
 */
export function simulateSignInAddTradeTouchSequence(scenario = {}) {
  const {
  ...
  if (signInAddTradeUsesTapUpActivation(pointerTypeDown, false)) {
    touchGestureActive = true
    for (const move of moves) {
      maxMovementPx = signInAddTradePeakMovementPx(startX, startY, move.x, move.y, maxMovementPx)
    }
    if (
      !cancelled
  ...
  const clickWouldAdd = signInAddTradeClickShouldAddRow({
    pointerType: clickPointerType,
    suppressSyntheticClick: suppressClick || suppressSyntheticClick,
    touchGestureActive,
  })
  if (clickWouldAdd) rowsAdded += 1
  return { rowsAdded, clickWouldAdd }
}

/**
 * Mouse/keyboard click may add a row; touch/pen synthetic click must not (handled on pointerup).
 * @param {{ pointerType?: unknown, suppressSyntheticClick?: boolean }} params
 */
export function signInAddTradeClickShouldAddRow({
  pointerType,
  suppressSyntheticClick = false,
  touchGestureActive = false,
} = {}) {
  if (suppressSyntheticClick || touchGestureActive === true) return false
  const type = String(pointerType || '')
  if (type === 'touch' || type === 'pen') return false
  return true
}
