 ...
 *
 * Used by Setup “Edit Report Details” and diary “Edit This Diary” loaders.
 */

import { resolveProjectReference } from './project-sticky-fields.js'
import { coverPhotoStateFromSaved } from './diary-cover-photo.js'
...
export function coverStoragePathFromReport(report) {
  if (!report || report.cover_photo_url == null) return null
  const path = String(report.cover_photo_url).trim()
  return path || null
}
...
/**
 * Edit Report Details URL — always carries report + project for setup hydrate.
 */
export function editReportDetailsHref(projectId, reportId) {
...
/**
 * Edit This Diary keeps the same report; setup details use editReportDetailsHref.
 */
...
export function projectHydrateSelectColumns({ includeProjectReference = true } = {}) {
  const sticky = includeProjectReference
    ? 'site_address, client_pm, working_days_per_week, current_phase, project_reference'
...
/**
 * Load report + project for Edit Report Details / edit-mode hydrate.
