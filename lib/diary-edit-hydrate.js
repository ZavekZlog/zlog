...
/**
 * Workbench load watchdog — critical hydrate success must not be overridden by a
 * late timer, and stale timeout errors must clear when the same generation succeeds.
 *
 * @param {{
 *   timeoutMs?: number,
 ...
 * }} [options]
 */
export function createDiaryWorkbenchLoadWatchdog({
  ...
  const timer = setTimeout(() => {
    ...
    if (criticalHydrateSucceeded) return
    timedOut = true
    onTimeout()
  }, timeoutMs)

  return {
    markCriticalHydrateSucceeded() {
      if (!isActiveGeneration()) return
      criticalHydrateSucceeded = true
    },
    shouldApplyLoadFailure() {
      if (!isActiveGeneration()) return false
      if (criticalHydrateSucceeded) return false
      return true
    },
    shouldClearStaleLoadError() {
      if (!isActiveGeneration()) return false
      return criticalHydrateSucceeded
    },
    clear() {
      clearTimeout(timer)
    },
    ...
    get criticalHydrateSucceeded() {
      return criticalHydrateSucceeded
    },
  }
}

/** Sign-in sheet preview / evidence URL hydrate must not gate diary open. */
export function isCriticalDiaryWorkbenchHydrateComplete(watchdog) {
  return Boolean(watchdog?.criticalHydrateSucceeded)
}
