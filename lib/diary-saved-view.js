 ...
 *
 * First paint waits for report, project, reporting company, child rows,
 * storage paths, and the complete SDSC seed.
 * Cover, signature, and photo thumbnail signing are deferred to hydrateDisplayMedia.
 */
export async function loadSavedDiaryView(supabase, { projectId = null, reportId = null } = {}) {
  ...
  const companyPromise = fetchReportingCompanyForReport(supabase, report)
  ...
  const view = buildSavedDiaryView({
    ...viewInputs,
    photoAreas: savedDiaryPhotoAreas(unsignedPhotos),
  })
  ...
  const hydratePhotos = async () => {
    try {
      const signedPhotos = await signSavedPhotoGridRows(photoRows || [], {
        session: createSavedPhotoSignSession(supabase),
        mapRow: (row, _index, signed) => mapSavedPhotoGridRow(row, signed),
      })
      return { photoAreas: savedDiaryPhotoAreas(signedPhotos) }
    } catch {
      return { photoAreas: view.photoAreas }
    }
  }
  ...
  const failedPhotosPatch = () => ({ photoAreas: view.photoAreas })
