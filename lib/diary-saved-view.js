 ...
 * (components/site-diary/SiteDiaryWorkbenchSurface.jsx) only ever renders the fields it
...
import { labourAggregateTotals } from './labour-from-register.js'
import {
  SIGN_IN_SHEET_EVIDENCE_PREVIEW_LOAD_FAIL_MESSAGE,
  signInSheetPathFromReport,
} from './diary-sign-in-sheet-evidence.js'
import { loadSignInSheetPreparedEvidence } from './diary-sign-in-sheet-session-cache.js'
import { signedUrlForPath } from './diary-share-pdf-assets.js'
import {
...
import { gridImageSrc } from './photo-workspace/thumbnail-display.js'
...
function labourRowsFromDb(rows) {
  ...
  })).filter((r) => r.trade || r.headcount || r.hours)
}
...
/** Workers + hours totals — same aggregation as the edit workbench. */
export function savedDiaryLabourTotals(labourRows = []) {
  return labourAggregateTotals(labourRows)
}
...
export function buildSavedDiaryView({
  ...
  const labourTotals = savedDiaryLabourTotals(labourRows)
  const reportDate = toDateInputValue(report?.report_date) || null
  ...
  const attendanceRegisterPath = signInSheetPathFromReport(report)
  const attendanceRegisterPreviewStatus = attendanceRegisterPath
  ...
  return {
    ...
    labour: labourRows,
    labourTotal: labourTotals.operatives,
    labourTotals,
    attendanceRegisterPath,
    attendanceRegisterPreviewUrl: null,
    attendanceRegisterPreviewStatus,
...
export async function loadSavedDiaryView(supabase, { projectId = null, reportId = null } = {}) {
  ...
  if (routeProjectId && report.project_id && String(report.project_id) !== String(routeProjectId)) {
    endDiaryHydrationTiming('report-not-found')
    return savedDiaryOpenFailure('report-not-found')
  }
  ...
  const runHydrateSecondary = async (onPatch) => {
    ...
    markDiaryHydrationTiming(DIARY_HYDRATION_STAGE.H2, {
      ...countPipelinePhotos(photoRows || []),
    })
    const unsignedPhotos = (photoRows || []).map((row) => mapSavedPhotoGridRow(row))
    lastPhotoAreas = savedDiaryPhotoAreas(unsignedPhotos)
    const unsignedWithSrc = unsignedPhotos.filter((row) => Boolean(gridImageSrc(row))).length
    markDiaryHydrationTiming(DIARY_HYDRATION_STAGE.H5, {
      phase: 'saved-secondary-metadata',
      photoCount: unsignedPhotos.length,
      withDisplaySrc: unsignedWithSrc,
    })
    const labourRows = labourRowsFromDb(labour)
    const labourTotals = savedDiaryLabourTotals(labourRows)
  ...
  const hydrateCover = async () => {
    try {
      ...
      markDiaryHydrationTiming(DIARY_HYDRATION_STAGE.H8, {
        hasPreview: Boolean(coverPhotoUrl),
        media: 'cover',
      })
      return {
        coverPhotoUrl,
  ...
  const hydrateSignature = async () => {
    try {
      ...
      markDiaryHydrationTiming(DIARY_HYDRATION_STAGE.H10, {
        hasPreview: Boolean(signatureUrl),
        media: 'signature',
      })
      return {
        signatureUrl,
  ...
  const signInSheetStoragePath = signInSheetPathFromReport(report)

  const hydrateAttendanceRegister = async () => {
    const path = signInSheetStoragePath
    if (!path) {
      return {
        attendanceRegisterPath: null,
        attendanceRegisterPreviewUrl: null,
        attendanceRegisterPreviewStatus: SAVED_DIARY_MEDIA_ABSENT,
        attendanceRegisterLoadError: '',
      }
    }
    try {
      const { blob } = await loadSignInSheetPreparedEvidence(path, {
        signedUrlForPath,
        supabase,
      })
      if (!blob || blob.size < 1) {
        throw new Error('empty-attendance-blob')
      }
      const previewUrl = URL.createObjectURL(blob)
      return {
        attendanceRegisterPath: path,
        attendanceRegisterPreviewUrl: previewUrl,
        attendanceRegisterPreviewStatus: SAVED_DIARY_MEDIA_READY,
        attendanceRegisterLoadError: '',
      }
    } catch {
      return {
        ...
        attendanceRegisterLoadError: SIGN_IN_SHEET_EVIDENCE_PREVIEW_LOAD_FAIL_MESSAGE,
      }
    }
  }
  ...
  const hydratePhotos = async () => {
    try {
      ...
      const flat = lastPhotoAreas.flatMap((area) => area.photos || [])
      const withDisplaySrc = flat.filter((row) => Boolean(gridImageSrc(row))).length
      markDiaryHydrationTiming(DIARY_HYDRATION_STAGE.H5, {
        phase: 'saved-display-media-signed',
        photoCount: flat.length,
        withDisplaySrc,
      })
  ...
  const runHydrateDisplayMedia = async (onPatch) => {
    ...
    const attendanceTask = Promise.resolve()
      .then(() => hydrateAttendanceRegister())
      .then(notify, () => notify({
        attendanceRegisterPath: signInSheetStoragePath,
        attendanceRegisterPreviewUrl: null,
        attendanceRegisterPreviewStatus: signInSheetStoragePath
          ? SAVED_DIARY_MEDIA_FAILED
          : SAVED_DIARY_MEDIA_ABSENT,
        attendanceRegisterLoadError: signInSheetStoragePath
          ? SIGN_IN_SHEET_EVIDENCE_PREVIEW_LOAD_FAIL_MESSAGE
          : '',
      }))

    const settled = await Promise.allSettled([coverTask, signatureTask, photosTask, attendanceTask])
    const merged = {}
    ...
    markDiaryHydrationTiming(DIARY_HYDRATION_STAGE.H12, {
      surface: 'saved-diary-view',
      displayMediaTasks: settled.length,
    })
    return merged
  }
  ...
  return {
    ...
    hydrateDisplayMedia: {
      ...
      attendanceRegister: hydrateAttendanceRegister,
