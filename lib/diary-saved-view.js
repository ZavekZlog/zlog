...
import { coverStoragePathFromReport, loadEditDiarySetupSources } from './diary-edit-hydrate.js'
...
export async function loadSavedDiaryView(supabase, { projectId = null, reportId = null } = {}) {
  ...
  const signSession = createPhotoDisplaySignSession({
    expiresIn: 3600,
  ...
  // Canonical cover path from the saved row (same field edit + PDF use).
  const coverPhotoPath =
    sources.hydration?.coverStoragePath || coverStoragePathFromReport(report)

  const [coverPhotoUrl, signatureUrl] = await Promise.all([
    resolveCoverPhotoPreviewUrl(supabase, coverPhotoPath),
  ...
  return {
    ...
    sdscSeed: buildSavedDiarySdscSeed({
      report,
      project: sources.project,
      projectId: resolvedProjectId,
      hydration: sources.hydration,
      company,
    }),
  }
}
