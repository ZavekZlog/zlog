...
import {
  DISPLAY_SIGNED_URL_DEFAULT_EXPIRES_SEC,
  getValidDisplaySignedUrl,
  storeDisplaySignedUrl,
} from './photo-workspace/display-signed-url-cache.js'

export { preparedCoverStoragePath, rawCoverStoragePath, ZLOG_COVER_PIPELINE_ID }

/** PDF embed caps only — stored cover pipeline (2400px) unchanged. */
export const PDF_COVER_EMBED_MAX_EDGE = 1600
export const PDF_COVER_EMBED_JPEG_QUALITY = 0.85
...
/**
 * True when a durable cover exists but is not a canonical prepared JPEG.
 * Prepared `covers/{generation}.jpg` is never eligible. Missing cover is not eligible.
 * http(s) stored URLs are treated as legacy, never as prepared identity.
 */
export function isLegacyCoverEligibleForMigration(coverPath) {
  const raw = normalizeCoverStoragePath(coverPath)
  if (!raw) return false
  if (raw.startsWith('data:') || raw.startsWith('blob:')) return false
  if (isPreparedCoverStoragePath(raw)) return false
  return true
}

/**
 * First storage-path segment (`{userId}/…`). Null for absolute URLs.
 */
export function coverOwnerUserIdFromPath(coverPath) {
  const raw = normalizeCoverStoragePath(coverPath)
  if (!raw) return null
  if (/^https?:\/\//i.test(raw) || raw.startsWith('data:') || raw.startsWith('blob:')) {
    return null
...
function newCoverMigrationGeneration() {
...
async function resolveCoverMigrationUserId(supabase, coverPath, explicitUserId) {
  if (explicitUserId) return String(explicitUserId)
  const fromPath = coverOwnerUserIdFromPath(coverPath)
...
async function fetchCoverSourceBlobForMigration(signedCoverUrl, fetchFn) {
  if (!signedCoverUrl || typeof fetchFn !== 'function') return null
...
async function persistMigratedCoverMetadata(supabase, {
  reportId,
  previousCoverPath,
  storagePath,
} = {}) {
  if (!supabase || !reportId || !previousCoverPath || !storagePath) {
    return { ok: false }
  }
  const { data, error } = await supabase
    ...
    .eq('id', reportId)
    .eq('cover_photo_url', previousCoverPath)
    .select('id, cover_photo_url, cover_processing_version')
    .maybeSingle()

  if (error || !data?.id) return { ok: false }
  if (String(data.cover_photo_url || '') !== String(storagePath)) return { ok: false }
  return { ok: true }
}

function emptyCoverMigrationCounts(needed) {
  return {
    coverMigrationNeeded: needed === true,
...
/**
 * One-time legacy/raw cover → canonical prepared JPEG for the CURRENT report only.
 * Does not delete the legacy object. Does not use raw-fallback persist.
 * Fail closed: leave the existing cover reference unchanged.
 *
 * @returns {Promise<{
 *   ok: boolean,
 *   skipped: boolean,
 ...
 * }>}
 */
export async function migrateLegacyCoverIfNeeded(supabase, {
  reportId,
  coverPath = null,
  ...
  const originalPath = coverPath == null || coverPath === '' ? null : String(coverPath)
  const normalizedPath = normalizeCoverStoragePath(originalPath)
  const existingLocal =
    localPreparedBlob instanceof Blob && localPreparedBlob.size > 0
      ? localPreparedBlob
      : null
  const needed = isLegacyCoverEligibleForMigration(originalPath)
  patchShareTimingCounts(emptyCoverMigrationCounts(needed))

  const unchanged = {
    ok: false,
    skipped: !needed,
    reason: needed ? 'pending' : (normalizedPath ? 'already-prepared' : 'no-cover'),
    coverPath: originalPath,
    coverProcessingVersion: coverProcessingVersion ?? null,
    localPreparedBlob: existingLocal,
  }

  if (!needed) return unchanged

  try {
    const ownerId = await resolveCoverMigrationUserId(supabase, normalizedPath, userId)
    if (!supabase || !ownerId || !reportId || !originalPath) {
      return { ...unchanged, skipped: false, reason: 'missing-args' }
    }

    const sourceBlob = await fetchCoverSourceBlobForMigration(signedCoverUrl, fetchFn)
    if (!sourceBlob) {
      return { ...unchanged, skipped: false, reason: 'fetch-failed' }
    }

    let prepared = null
    try {
      prepared = await prepareFn(sourceBlob)
    } catch {
      return { ...unchanged, skipped: false, reason: 'prepare-failed' }
    }
    const preparedBlob = prepared?.blob
    if (!(preparedBlob instanceof Blob) || preparedBlob.size < 1) {
      return { ...unchanged, skipped: false, reason: 'prepare-failed' }
    }
    patchShareTimingCounts({ coverMigrationPrepareCount: 1 })

    const uploaded = await uploadPreparedCoverFile(supabase, {
      userId: ownerId,
      reportId,
      generation: generation || newCoverMigrationGeneration(),
      file: preparedBlob,
    })
    if (uploaded.error || !uploaded.storagePath) {
      return { ...unchanged, skipped: false, reason: 'upload-failed' }
    }
    patchShareTimingCounts({ coverMigrationUploadCount: 1 })

    const persisted = await persistMigratedCoverMetadata(supabase, {
      reportId,
      previousCoverPath: originalPath,
      storagePath: uploaded.storagePath,
    })
    if (!persisted.ok) {
      return { ...unchanged, skipped: false, reason: 'persist-failed' }
    }
    patchShareTimingCounts({ coverMigrationPersistCount: 1 })
    ...
    return {
      ...
      localPreparedBlob: preparedBlob,
    }
  } catch {
    return { ...unchanged, skipped: false, reason: 'migration-failed' }
  }
}
...
function rememberPreparedCoverSessionBlob(identity, blob) {
  const beforeEvict = getPreparedCoverSessionCacheStats().evictCount
  const stored = storePreparedCoverSessionBlob(identity, blob)
  if (!stored) return
  bumpShareTimingCountSilent('coverSessionBlobCacheStoreCount')
  const evicted = Math.max(0, getPreparedCoverSessionCacheStats().evictCount - beforeEvict)
  if (evicted) bumpShareTimingCountSilent('coverSessionBlobCacheEvictCount', evicted)
}

function publishPreparedCoverSessionCacheBytes() {
  patchShareTimingCounts({
    coverSessionBlobCacheBytes: getPreparedCoverSessionCacheStats().bytes,
  })
}

/**
 * PDF cover source: prepared JPEG resized for PDF embed only (local/session/network blob);
 ...
 *   embedPreparedCoverBlob?: (blob: Blob) => Promise<string|null>,
 * }} options
 */
export async function resolveCoverPdfSource(signedCoverUrl, {
  ...
  localPreparedBlob = null,
  uprightCoverFn,
  embedPreparedCoverBlob = null,
} = {}) {
  async function embedPreparedCover(blob) {
    if (typeof embedPreparedCoverBlob === 'function') {
      return embedPreparedCoverBlob(blob)
    }
  ...
  if (prepared && localPreparedBlob instanceof Blob && localPreparedBlob.size > 0) {
    rememberPreparedCoverSessionBlob(identity, localPreparedBlob)
    patchShareTimingCounts({
      coverPreparedSource: 'local',
      coverNetworkFetchCount: 0,
      coverOrientationBakeCount: typeof embedPreparedCoverBlob === 'function' ? 1 : 0,
      coverPassThroughCount: 1,
    })
    publishPreparedCoverSessionCacheBytes()
    const dataUrl = await embedPreparedCover(localPreparedBlob)
    markShareTiming('pdf_cover_source_done')
    return dataUrl
  }

  if (prepared) {
    let sessionBlob = lookupPreparedCoverSessionBlob(identity)
    if (!sessionBlob) {
    ...
    if (sessionBlob) {
      bumpShareTimingCountSilent('coverSessionBlobCacheHitCount')
      patchShareTimingCounts({
        coverPreparedSource: 'session',
        coverNetworkFetchCount: 0,
        coverOrientationBakeCount: typeof embedPreparedCoverBlob === 'function' ? 1 : 0,
        coverPassThroughCount: 1,
      })
      publishPreparedCoverSessionCacheBytes()
      const dataUrl = await embedPreparedCover(sessionBlob)
      markShareTiming('pdf_cover_source_done')
      return dataUrl
    }
    bumpShareTimingCountSilent('coverSessionBlobCacheMissCount')
    if (!signedCoverUrl) {
      patchShareTimingCounts({
    ...
    rememberPreparedCoverSessionBlob(identity, blob)
    patchShareTimingCounts({
      coverPreparedSource: usedPdfNetworkFetch ? 'network' : 'session',
      coverNetworkFetchCount: usedPdfNetworkFetch ? 1 : 0,
      coverOrientationBakeCount: 0,
      coverPassThroughCount: 1,
    })
    ...
    publishPreparedCoverSessionCacheBytes()
    const dataUrl = await embedPreparedCover(blob)
    if (typeof embedPreparedCoverBlob === 'function') {
      patchShareTimingCounts({ coverOrientationBakeCount: 1 })
    }
    markShareTiming('pdf_cover_source_done')
    return dataUrl
  }
...
export async function resolveCoverPhotoPreviewUrl(supabase, path) {
  ...
  const cached = getValidDisplaySignedUrl(raw, { recordStats: true })
  if (cached) return cached
  try {
    const expiresIn = DISPLAY_SIGNED_URL_DEFAULT_EXPIRES_SEC
    const { data, error } = await supabase.storage.from('site-photos').createSignedUrl(raw, expiresIn)
    ...
    if (url) storeDisplaySignedUrl(raw, url, expiresIn)
    return url
  } catch {
    return null
  }
