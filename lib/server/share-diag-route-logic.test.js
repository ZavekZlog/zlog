...
describe('share-diag route logic', () => {
  ...
  it('preserves edit-navigation correlation fields', () => {
    const safe = sanitizeShareDiagPayloadForLog({
      stage: 'edit-navigation-start',
      reportId: '6846b690-b3f8-4a28-b0c1-a9a825a850f0',
      projectId: '650730f3-684d-425f-916b-a5811edd70f3',
      surface: 'saved-diary-view',
      hydrationSessionId: '1758789410170-abc1234',
      elapsedMs: 0,
      href: 'https://preview.example/diary/view?report=leak',
      siteSummary: 'secret diary text',
    })
    assert.equal(safe.stage, 'edit-navigation-start')
    assert.equal(safe.surface, 'saved-diary-view')
    assert.equal(safe.hydrationSessionId, '1758789410170-abc1234')
    assert.equal(safe.elapsedMs, 0)
    assert.equal(safe.href, undefined)
    assert.equal(safe.siteSummary, undefined)
  })
