...
export async function assembleSiteDiaryPdfDocumentProps(admin, reportId) {
  ...
  const { data: plantRows } = await admin
    ...
    .eq('report_id', reportId)
    .order('sequence')
  timings.reportDataMs += nowMs() - labourPlantT0
  ...
  let attendanceRegisterImageSrc = null
  if (signInSheetStoragePath && isSafeSignInSheetCleanupPath(signInSheetStoragePath)) {
    const attT0 = nowMs()
    const signedRegisterUrl = await signedUrlForPath(admin, signInSheetStoragePath)
    if (signedRegisterUrl) {
      if (String(signedRegisterUrl).startsWith('data:')) {
        attendanceRegisterImageSrc = signedRegisterUrl
      } else {
        const blob = await fetchHttpBlob(signedRegisterUrl)
        attendanceRegisterImageSrc = await uprightJpegBlobToPdfDataUrl(blob, 2400)
      }
    }
    timings.attendanceMs += nowMs() - attT0
  }
  ...
  const props = {
    ...
    plant: plantRows || [],
    equipmentHire,
    hsIncidents: report.hs_incidents,
    rfis: report.rfis,
    variations: report.variations,
    visitors: report.visitors,
    visitorsRegisterProvenance: report.visitors_register_provenance,
