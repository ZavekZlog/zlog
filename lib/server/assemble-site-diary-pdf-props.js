...
import { PDF_WORK_PHOTO_EMBED_JPEG_QUALITY } from '@/lib/diary-pdf-photos.js'
...
export async function assembleSiteDiaryPdfDocumentProps(admin, reportId) {
  ...
  async function uprightCoverForServer(signedCoverUrl) {
    ...
    try {
      ...
      const resolved = await resolveCoverPdfSource(signedCoverUrl, {
        ...
        uprightCoverFn: (url) =>
          uprightSignedUrlToPdfDataUrl(url, PDF_COVER_EMBED_MAX_EDGE, PDF_COVER_EMBED_JPEG_QUALITY),
        embedPreparedCoverBlob: (blob) =>
          uprightJpegBlobToPdfDataUrl(blob, PDF_COVER_EMBED_MAX_EDGE, PDF_COVER_EMBED_JPEG_QUALITY),
      })
      return resolved
    } finally {
  ...
  const photos = await buildDiaryPdfPhotos(
    ...
    async (photo) => signedUrlForPath(admin, photo.url),
    {
      ...
      serverImageFlatten: async ({
        baseSrc,
        maxEdge,
        localBlob,
        rotationDegrees,
        jpegQuality = PDF_WORK_PHOTO_EMBED_JPEG_QUALITY,
      }) => {
        ...
        if (!blob) return null
        let dataUrl = await uprightJpegBlobToPdfDataUrl(blob, maxEdge, jpegQuality)
        dataUrl = await applyRotationToPdfDataUrl(dataUrl, rotationDegrees, jpegQuality)
        return dataUrl
      },
    },
  )
