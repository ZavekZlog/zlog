...
describe('assembleSiteDiaryPdfDocumentProps — client PDF prop parity', () => {
  ...
  it('wires PDF-only cover and work-photo embed resize caps', () => {
    const source = readAssemblerSource()
    const pdfImage = readFileSync(join(root, 'lib/server/pdf-image-server.js'), 'utf8')
    assert.match(source, /PDF_COVER_EMBED_MAX_EDGE/)
    assert.match(source, /PDF_COVER_EMBED_JPEG_QUALITY/)
    assert.match(source, /PDF_WORK_PHOTO_EMBED_JPEG_QUALITY/)
    assert.match(source, /embedPreparedCoverBlob/)
    assert.match(source, /serverImageFlatten/)
    assert.match(pdfImage, /DEFAULT_PDF_JPEG_QUALITY = 92/)
  })
