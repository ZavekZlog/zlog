...
import { finalizeLabourOnly } from './diary-save.js'
import { labourFormToPersistRows } from './diary-save-dirty.js'

export const LABOUR_APPLY_SAVE_FAIL_MESSAGE =
  'We couldn’t save the Labour Attendance Summary. The figures are still on this screen. Check your connection and tap Apply again.'
...
/**
 * Replace report_labour for this diary with the applied form rows (atomic RPC).
 * @returns {Promise<object[]>} persist-shaped rows (count / hours / sequence)
 */
export async function persistAppliedLabourRows(supabase, reportId, projectId, formRows) {
  if (!reportId) {
    throw new Error('missing-report-id')
  }
  const labourPayload = labourFormToPersistRows(formRows, reportId)
  await finalizeLabourOnly(supabase, reportId, projectId, labourPayload)
