...
describe('diary hydration timing diagnostics', () => {
  ...
  it('honours an external hydrationSessionId when provided', () => {
    const session = beginDiaryHydrationTiming({
      surface: 'workbench',
      reportId: 'r1',
      projectId: 'p1',
      hydrationSessionId: 'edit-nav-session-xyz',
    })
    assert.equal(session.id, 'edit-nav-session-xyz')
    endDiaryHydrationTiming('test')
  })
