...
describe('New Site Diary from scratch — clean initial state', () => {
  ...
  it('3. Author and latest Reporting On Behalf Of are editable defaults', () => {
    const scratch = blankDiarySetupFormState({
      authorName: 'Signed In User',
      reportingOnBehalfOf: 'Turner Construction',
      reportDate: '2026-07-31',
    })
    ...
    assert.equal(scratch.reportingOnBehalfOf, 'Turner Construction')
    assert.equal(scratch.projectManager, '')
    assert.equal(scratch.workingDaysPerWeek, '5')
  })
  ...
  it('9. selecting an existing project loads project fields but not Current Phase', () => {
  ...
  it('10. switching back to New project clears them again', () => {
    ...
    assert.equal(cleared.workingDaysPerWeek, '5')
    assert.equal(cleared.reportingOnBehalfOf, 'From Latest Diary Co')
...
describe('scratch setup wiring — no session / prior diary bleed', () => {
  ...
  it('Reporting On Behalf Of prefills from the explicit latest value, not company name', () => {
    const state = initialiseNewDiarySetupState({
      authorName: 'Alex',
      reportingOnBehalfOf: 'Turner Construction',
      companyProfile: { id: 'co-1', company_name: 'Acme Construction', brand_color: '#0a0' },
    })
    assert.equal(state.reportingOnBehalfOf, 'Turner Construction')
  })
...
describe('PROTECTED — scratch Author Name from signed-in profile', () => {
  ...
  it('Project & Report Details still hydrates Author Name from saved diary', () => {
    assert.match(setupPage, /loadEditDiarySetupSources/)
