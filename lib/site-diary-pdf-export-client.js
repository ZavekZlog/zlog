...
/** Post-hydrate reconcile diagnostics (Preview-safe; no signed URLs). */
export const SITE_DIARY_PDF_RECONCILE_DIAG_STAGE = {
  r1Start: 'post-hydrate-reconcile-r1-start',
  r2Fingerprint: 'post-hydrate-reconcile-r2-fingerprint-complete',
  r3Enqueue: 'post-hydrate-reconcile-r3-enqueue',
  r4ArtifactAuthorizeStart: 'post-hydrate-reconcile-r4-artifact-authorize-start',
  r5ArtifactAuthorizeComplete: 'post-hydrate-reconcile-r5-artifact-authorize-complete',
  r6SignedFetchResponse: 'post-hydrate-reconcile-r6-signed-fetch-response',
  r7SignedFetchBlob: 'post-hydrate-reconcile-r7-signed-fetch-blob',
  r8ShareReadyBuilt: 'post-hydrate-reconcile-r8-share-ready-built',
  r9Complete: 'post-hydrate-reconcile-r9-complete',
  r9Discarded: 'post-hydrate-reconcile-r9-discarded',
}
...
/**
 * @param {string} stage
 * @param {number | undefined} reconcileStartedAt
 * @param {Record<string, unknown>} payload
 * @param {{ onReconcileDiag?: (stage: string, payload: Record<string, unknown>) => void }} [options]
 */
function emitReconcileDiagStage(stage, reconcileStartedAt, payload, options = {}) {
  const started = Number(reconcileStartedAt)
  const timingPayload = {
    ...payload,
    tapStartedAt: Number.isFinite(started) ? started : undefined,
    elapsedMsSinceTap: Number.isFinite(started) ? Date.now() - started : undefined,
  }
  if (typeof options.onReconcileDiag === 'function') {
    options.onReconcileDiag(stage, timingPayload)
    return
  }
...
export async function fetchSiteDiaryPdfExportShareReadyArtifact(
  ...
  if (reconcileStartedAt != null || onReconcileDiag) {
    emitReconcileDiagStage(
      SITE_DIARY_PDF_RECONCILE_DIAG_STAGE.r4ArtifactAuthorizeStart,
      reconcileStartedAt,
      { reportId: trimString(reportId), exportId: trimString(exportId) },
      { onReconcileDiag },
    )
  }

  const authorized = await fetchSiteDiaryPdfExportAuthorization(reportId, exportId, {
    signal,
    fetch: fetchFn,
    tapStartedAt,
    onShareTiming,
  ...
  if (reconcileStartedAt != null || onReconcileDiag) {
    emitReconcileDiagStage(
      SITE_DIARY_PDF_RECONCILE_DIAG_STAGE.r5ArtifactAuthorizeComplete,
      reconcileStartedAt,
      {
  ...
  if (reconcileStartedAt != null || onReconcileDiag) {
    emitReconcileDiagStage(
      SITE_DIARY_PDF_RECONCILE_DIAG_STAGE.r6SignedFetchResponse,
      reconcileStartedAt,
      {
        reportId: rid,
        exportId: eid,
  ...
  if (reconcileStartedAt != null || onReconcileDiag) {
    emitReconcileDiagStage(
      SITE_DIARY_PDF_RECONCILE_DIAG_STAGE.r7SignedFetchBlob,
      reconcileStartedAt,
      {
        reportId: rid,
        exportId: eid,
  ...
  const shareReady = buildShareReadySiteDiaryPdfFromBlob(blob, {
    fileName,
    title,
    text,
  })
  ...
  if (reconcileStartedAt != null || onReconcileDiag) {
    emitReconcileDiagStage(
      SITE_DIARY_PDF_RECONCILE_DIAG_STAGE.r8ShareReadyBuilt,
      reconcileStartedAt,
      {
        reportId: rid,
        exportId: eid,
...
/**
 * Authoritative export identity only: fingerprint → idempotent enqueue (no artifact download).
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string} reportId
 * @param {{
 ...
 *   onReconcileDiag?: (stage: string, payload: Record<string, unknown>) => void
 *   tapStartedAt?: number
 *   onShareTiming?: ShareTimingHook
 * }} [options]
 */
export async function resolveAuthoritativeSiteDiaryPdfExportIdentity(
  supabase,
  reportId,
  options = {},
) {
  const reconcileStartedAt = options.reconcileStartedAt ?? Date.now()
  const { isStillValid } = options
  const rid = trimString(reportId)

  emitReconcileDiagStage(
    SITE_DIARY_PDF_RECONCILE_DIAG_STAGE.r1Start,
    reconcileStartedAt,
    { reportId: rid },
    options,
  )
  ...
  emitReconcileDiagStage(
    SITE_DIARY_PDF_RECONCILE_DIAG_STAGE.r2Fingerprint,
    reconcileStartedAt,
    {
      reportId: fingerprint.reportId,
      fingerprintPrefix: shareTimingFingerprintPrefix(fingerprint.contentFingerprint),
  ...
  const enqueueStatus = trimString(enqueued.job?.status).toLowerCase()
  emitReconcileDiagStage(
    SITE_DIARY_PDF_RECONCILE_DIAG_STAGE.r3Enqueue,
    reconcileStartedAt,
    {
      reportId: fingerprint.reportId,
      exportId: enqueued.exportId,
  ...
  if (typeof isStillValid === 'function' && !isStillValid()) {
    emitReconcileDiagStage(
      SITE_DIARY_PDF_RECONCILE_DIAG_STAGE.r9Discarded,
      reconcileStartedAt,
      { reportId: fingerprint.reportId, reason: 'stale-after-enqueue' },
      options,
    )
    return {
      ok: false,
  ...
  const contentFingerprint =
    trimString(enqueued.job?.contentFingerprint).toLowerCase()
    || trimString(fingerprint.contentFingerprint).toLowerCase()
...
/**
 * Poll (if needed) and download share-ready artifact for a known export identity.
 * Does not enqueue.
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string} reportId
 * @param {{ ok: true, exportId: string, enqueueReturnedStatus?: string, contentFingerprint?: string, job?: object, reconcileStartedAt?: number }} identity
 * @param {Record<string, unknown>} [options]
 */
export async function hydrateShareReadyArtifactFromExportIdentity(
  supabase,
  reportId,
  identity,
  options = {},
  ...
  const rid = trimString(reportId) || trimString(identity.reportId)
  const exportId = trimString(identity.exportId)
  const reconcileStartedAt = identity.reconcileStartedAt ?? options.reconcileStartedAt
  const enqueueStatus = trimString(identity.enqueueReturnedStatus).toLowerCase()
  const { tapStartedAt, onShareTiming, signal } = options
  ...
  if (enqueueStatus === 'failed') {
    return {
      ok: false,
      code: SITE_DIARY_PDF_EXPORT_CLIENT_CODE.exportFailed,
      message: identity.job?.errorMessage || 'PDF export failed. Try again.',
      job: identity.job,
    }
  }
  ...
  if (enqueueStatus !== 'ready') {
    const polled = await pollSiteDiaryPdfExportJobUntilTerminal(supabase, exportId, {
      ...options,
      shareTimingEnqueueReturnedStatus: identity.enqueueReturnedStatus ?? null,
    })
    if (!polled.ok) {
      return polled
    }
  ...
  const artifact = await fetchSiteDiaryPdfExportShareReadyArtifact(rid, exportId, {
    ...options,
    reconcileStartedAt,
  })
  ...
  return {
    ...artifact,
    contentFingerprint: identity.contentFingerprint || null,
    enqueueReturnedStatus: enqueueStatus || identity.enqueueReturnedStatus || null,
  }
}

/**
 * Post-hydrate authoritative re-adoption: identity → artifact hydration.
...
export async function reconcileAuthoritativeSiteDiaryPdfExportToShareReady(
  ...
  const identity = await resolveAuthoritativeSiteDiaryPdfExportIdentity(supabase, reportId, {
    ...options,
    reconcileStartedAt,
  })
  ...
  const completed = await hydrateShareReadyArtifactFromExportIdentity(
    supabase,
    reportId,
    identity,
    options,
  ...
  if (typeof isStillValid === 'function' && !isStillValid()) {
    emitReconcileDiagStage(
      SITE_DIARY_PDF_RECONCILE_DIAG_STAGE.r9Discarded,
      reconcileStartedAt,
      { reportId: identity.reportId, reason: 'stale-after-hydrate' },
      options,
    )
    return {
      ok: false,
  ...
  emitReconcileDiagStage(
    SITE_DIARY_PDF_RECONCILE_DIAG_STAGE.r9Complete,
    reconcileStartedAt,
    {
