...
describe('diary setup continue CTA', () => {
  ...
  it('edit existing setup preserves coverPhotoUrl through updateDraft', async () => {
    const updates = []
    const result = await runDiarySetupContinue({
      form: validForm({
        coverPhotoUrl: 'user/rep-edit/cover.jpg',
    ...
    assert.equal(updates[0].fields.coverPhotoUrl, 'user/rep-edit/cover.jpg')
