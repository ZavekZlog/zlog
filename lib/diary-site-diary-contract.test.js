...
describe('6–8. Scratch / existing project / New project', () => {
  ...
  it('Dashboard opens a Site Diary hub of exactly two equal choices', () => {
    ...
    assert.match(
      hubPage,
      /Review a saved diary — or reuse one for today…/,
    )
    assert.match(hubPage, /Enter everything from scratch/)
  ...
  it('peer hub cards render at equal height without clipping copy', () => {
    ...
    assert.doesNotMatch(hubPage, /-webkit-line-clamp|textOverflow: 'ellipsis'/)
  })
  ...
  it('each saved-diary entry offers Open to review then Use for Today', () => {
    const cardActions = hubPage.slice(
      hubPage.indexOf('recentEntryActionsStyle,'),
      hubPage.indexOf('</RecentEntryCard>'),
    )

    assert.equal((cardActions.match(/<button\b/g) || []).length, 2)
    assert.ok(
      cardActions.indexOf('Open to review') < cardActions.indexOf('Use for Today'),
      'review is the primary intent of this list; reuse sits beside it',
    )
    assert.match(cardActions, /openExistingReport\(row\)/)
    assert.match(cardActions, /requestUsePreviousForToday\(row\)/)
    // Both actions share the row and wrap on narrow viewports.
    assert.match(cardActions, /flexWrap: 'wrap'/)
    assert.doesNotMatch(cardActions, /width:\s*'100%'/)
  })
