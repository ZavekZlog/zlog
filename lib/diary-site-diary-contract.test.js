...
import {
  DIARY_EDIT_CORE_DOM_MARKERS,
  DIARY_IN_APP_CORE_SECTION_ORDER,
  DIARY_SAVED_REVIEW_CORE_DOM_MARKERS,
} from './diary-section-order.js'
import {
...
const setupBehaviour = readSetupBehaviourSource()
const setupPage = readSetupUiComposedSource()
const diaryPage = readFileSync(join(root, 'components/site-diary/SiteDiaryWorkbenchSurface.jsx'), 'utf8')
const labourSection = readFileSync(
  join(root, 'components/diary/SiteDiaryLabourSection.jsx'),
  'utf8',
)
...
const screenContract = readFileSync(join(root, 'docs/contracts/SITE_DIARY_SCREEN_CONTRACT.md'), 'utf8')
...
describe('protected contract docs / rule', () => {
  it('authoritative contract hierarchy and Cursor rules exist', () => {
    ...
    assert.match(screenContract, /\*\*Version:\*\*\s*1\.22\.0/)
    assert.match(screenContract, /Continue to Today's Report/)
...
describe('Site Diary module accent identity', () => {
  it('uses the canonical diary accent throughout hub, setup, workbench and saved review states', () => {
    ...
    assert.match(setupHostPage, /import[\s\S]*DIARY_ACCENT[\s\S]*from '@\/lib\/premium-ui'/)
    assert.match(setupHostPage, /accent=\{DIARY_ACCENT\}/)
    assert.doesNotMatch(setupHostPage, /\bBRAND_ACCENT\b/)
...
describe('setup UI — live pre-audit label order (source still matches 1.21.0 until implementation)', () => {
  it('critical setup controls appear in contract order in page source', () => {
    ...
    const company = composed.indexOf('aria-label="Reporting Company Name"')
    const logo = composed.indexOf('>LOGO</label>')
    ...
    assert.ok(projectName > 0 && pm > projectName && projectName < company)
    assert.ok(datesMount > stickyMount && shift > datesMount)
    assert.ok(stickyMount > 0 && stickyMount < company, 'Project Manager stays inside Project Details before Reporting Company')
  ...
  it('live setup sections still render in pre-audit Project Details → Reporting Company → Behalf Of → Author → Cover photo order', () => {
    const stickySrc = readFileSync(
      join(root, 'components/project/ProjectStickyFields.jsx'),
      'utf8',
    )
    ...
    const companySection = setupPage.indexOf('aria-label="Reporting Company Name"')
    const behalfSection = setupPage.indexOf('title="Reporting On Behalf Of"')
    const authorSection = setupPage.indexOf('title="Author"')
    const coverSection = setupPage.indexOf('title="Cover photo"')
    const stickyMount = setupPage.indexOf('<ProjectStickyFields')
    const projectName = setupPage.indexOf('aria-label="Project Name"')
    ...
    assert.ok(projectName > projectSection && projectName < companySection, 'Project Name is first inside Project Details')
    assert.ok(stickyMount > projectSection && stickyMount < companySection, 'Project Manager fields mount inside Project Details')
    ...
    assert.match(setupPage, />LOGO</)
    assert.match(setupPage, /aria-label="Reporting Company Name"/)
  })
  ...
  it('user-visible setup copy never uses sticky/implementation terminology', () => {
    const setupSection = readSetupSectionSource()
    assert.doesNotMatch(setupSection, /Sticky project information/)
    assert.doesNotMatch(setupSection, /sticky fields/i)
    assert.doesNotMatch(setupSection, /persistent/i)
    assert.doesNotMatch(setupSection, /inherited values/i)
    assert.doesNotMatch(setupSection, /database fields/i)
    assert.doesNotMatch(setupSection, /cached project/i)
    assert.doesNotMatch(setupSection, /remembered for this project/i)
    assert.doesNotMatch(setupSection, /Programme dates for this project/)
    ...
    assert.match(setupHostPage, /Confirm the details for today’s Site Diary, then continue/)
    assert.match(setupHostPage, /Project & Report Details/)
    assert.match(setupHostPage, /Review the saved project and report details/)
    assert.match(setupHostPage, /Continue to Today's Diary/)
    assert.match(screenContract, /Continue to Today's Report/)
    assert.doesNotMatch(screenContract, /New-diary primary CTA: \*\*Continue to Site Diary\*\*/)
...
describe('1–2. Shift selector on setup UI (labels + order)', () => {
  ...
  it('Shift appears in Project Details after programme dates', () => {
    ...
    const role = setupPage.indexOf('>Author Role</label>')
    const company = setupPage.indexOf('aria-label="Reporting Company Name"')
    assert.ok(author > 0 && role > author)
    assert.ok(projectName > 0 && projectName < company)
    assert.ok(dates > projectName && shift > dates && shift < company)
  })
})
...
describe('3–4. Author Name and Author Role on setup UI', () => {
  ...
  it('Author Role exists directly after Author Name', () => {
    ...
    const projectName = setupPage.indexOf('Project Name')
    assert.ok(author > 0 && role > author)
    assert.ok(projectName > 0 && projectName < author, 'Project Name remains before Author')
    const between = setupPage.slice(author, role)
...
describe('6–8. Scratch / existing project / New project', () => {
  it('Start from scratch clears prior project and diary state', () => {
    ...
    assert.match(setupBehaviour, /initialiseNewDiarySetupState/)
  ...
  it('Start a New Diary prefills Author Name from signed-in profile only', () => {
    assert.match(setupBehaviour, /resolveSignedInAuthorProfile\(/)
    assert.match(setupBehaviour, /authorName:\s*profileName/)
    assert.match(setupBehaviour, /setAuthor\(fresh\.author\)/)
  ...
  it('Use a Previous Diary creates a new diary and does not update the source', () => {
    ...
    assert.match(savedDiaryPage, /createTodaysDiaryDraft/)
    // The new diary is reviewed on Project & Report Details before the workbench.
    assert.match(savedDiaryPage, /projectAndReportDetailsHref/)
    assert.doesNotMatch(savedDiaryPage, /diaryFormHref/)
  ...
  it('each compact saved-diary row is a single tap target that opens review', () => {
    const listRows = hubPage.slice(
      hubPage.indexOf('className="zlog-saved-diary-list"'),
      hubPage.indexOf('export default function SiteDiaryEntryRoute'),
    )
    ...
    assert.match(listRows, /zlog-saved-diary-project/)
    assert.match(listRows, /\{reportDate\} · Shift: \{shift\}/)
    assert.doesNotMatch(listRows, /Open to review/)
    assert.doesNotMatch(listRows, /Use for Today/)
    assert.doesNotMatch(listRows, /requestUsePreviousForToday/)
    assert.doesNotMatch(listRows, /requestDeleteDiary/)
    assert.doesNotMatch(listRows, /zlog-saved-diary-use/)
    assert.doesNotMatch(listRows, /zlog-saved-diary-delete/)
    assert.doesNotMatch(listRows, /zlog-saved-diary-open-label/)
    assert.doesNotMatch(listRows, /PrimaryCTA/)
    assert.doesNotMatch(listRows, /Delete Diary/)
    assert.match(listRows, /selectionMode/)
    assert.match(listRows, /type="checkbox"/)
    assert.match(listRows, /toggleSelected\(row\.id\)/)
    assert.match(listRows, /openExistingReport\(row\)/)

    const openRule = hubPage.slice(
      hubPage.indexOf('.zlog-saved-diary-open {'),
      hubPage.indexOf('.zlog-saved-diary-open:hover {'),
    )
    assert.match(openRule, /min-height:\s*72px/)
    assert.match(openRule, /cursor:\s*pointer/)
    assert.match(hubPage, /\.zlog-saved-diary-open:hover \{/)
    assert.match(hubPage, /\.zlog-saved-diary-open:active:not\(\[data-opening='true'\]\) \{/)
    assert.match(hubPage, /navigateToSavedDiaryViewer/)
    assert.match(hubPage, /\.zlog-saved-diary-open:focus-visible \{/)
  })
  ...
  it('Switching to New Project clears sticky project fields', () => {
    ...
    assert.match(setupBehaviour, /clearToNewProjectSelection/)
  })
})
...
describe('12–13. Project link and Project Day', () => {
  ...
  it('Site Diary workbench must not render a duplicate Project details recap card', () => {
    ...
    assert.match(readSetupSectionSource(), /Review \/ Edit Project & Report Details/)
  ...
  it('a compact divider separates Project & Report Details from today’s diary; Cover lives on inline details', () => {
    const summaryIdx = diaryPage.indexOf('projectProgrammeCard.projectDayLine')
    const dividerIdx = diaryPage.indexOf('todaysDiaryDividerTitleStyle}>', summaryIdx)
    assert.ok(summaryIdx > 0 && dividerIdx > summaryIdx, 'divider sits below the project summary card')
    assert.equal([...diaryPage.matchAll(/title="Cover photo"/g)].length, 0, 'Today’s Site Diary must not duplicate Cover photo')
    assert.equal([...setupPage.matchAll(/title="Cover photo"/g)].length, 1, 'Project & Report Details must contain Cover photo')
    assert.match(
      diaryPage,
  ...
  it('Cover photo control is on Project & Report Details — not under Today’s Site Diary', () => {
    assert.equal([...setupPage.matchAll(/title="Cover photo"/g)].length, 1)
    assert.equal([...diaryPage.matchAll(/title="Cover photo"/g)].length, 0)
    const dividerIdx = diaryPage.indexOf('todaysDiaryDividerTitleStyle}>')
    ...
    assert.ok(weatherIdx > dividerIdx, 'Weather is first diary-content card under Today’s')
  ...
  it('Report Date and Shift are not editable workbench controls (setup-only)', () => {
    ...
    // Weather is first diary-entry field under Today’s Site Diary (Cover is on setup).
    const weatherIdx = diaryPage.indexOf('title="Weather"')
    const labourMountIdx = diaryPage.indexOf('<SiteDiaryLabourSection')
    const labourTitleIdx = labourSection.indexOf('title="Labour"')
    const summaryIdx = diaryPage.indexOf('title="Site summary"')
    const dailyIdx = diaryPage.indexOf('<DiaryDailyRecordSections')
    const dividerIdx = diaryPage.indexOf('todaysDiaryDividerTitleStyle}>')
    assert.ok(weatherIdx > dividerIdx)
    assert.ok(labourMountIdx > weatherIdx)
    assert.ok(labourTitleIdx >= 0)
    assert.ok(labourMountIdx < dailyIdx && labourMountIdx < summaryIdx)
    assert.equal([...setupPage.matchAll(/title="Cover photo"/g)].length, 1)
    assert.equal([...diaryPage.matchAll(/title="Cover photo"/g)].length, 0)
  })

  it('locks Labour Attendance Summary before Attendance Register evidence in manual mode', () => {
    const summaryIdx = labourSection.indexOf('Labour Attendance Summary')
    const registerIdx = labourSection.indexOf('alt="Attendance register preview"')
    assert.ok(summaryIdx > 0 && registerIdx > summaryIdx)
  })

  it('renders Attendance Register inside Labour (no separate top-level register card on workbench)', () => {
    assert.equal([...labourSection.matchAll(/<GlassSection/g)].length, 1)
    assert.match(labourSection, /title="Labour"/)
    assert.doesNotMatch(labourSection, /title="Attendance Register"/)
    assert.match(labourSection, /Attendance Register\s*<\/p>/)
    const labourPanelClose = labourSection.lastIndexOf('</GlassSection>')
    const registerHeadingIdx = labourSection.indexOf('labourInternalHeadingStyle}>Attendance Register')
    assert.ok(registerHeadingIdx > 0 && registerHeadingIdx < labourPanelClose)
    assert.doesNotMatch(diaryPage, /title="Attendance Register"/)
  })

  it('physically orders Visitors JSX before H&S host on the workbench page', () => {
    const labourMountIdx = diaryPage.indexOf('<SiteDiaryLabourSection')
    const labourMountEnd = diaryPage.indexOf('/>', labourMountIdx)
    const visitorsTitle = diaryPage.indexOf(DIARY_EDIT_CORE_DOM_MARKERS.visitors, labourMountEnd)
    const hsHost = diaryPage.indexOf(DIARY_EDIT_CORE_DOM_MARKERS.hsHost, visitorsTitle)
    assert.ok(labourMountEnd > labourMountIdx)
    assert.ok(visitorsTitle > labourMountEnd, 'Visitors GlassSection must follow Labour mount')
    assert.ok(hsHost > visitorsTitle, 'DiaryDailyRecordSections must follow Visitors')
  })

  it('locks Weather → Labour → Visitors → H&S in compose/edit workbench', () => {
    const dailySections = readFileSync(
      join(root, 'components/diary/DiaryDailyRecordSections.jsx'),
      'utf8',
    )
    const editWeather = diaryPage.indexOf(DIARY_EDIT_CORE_DOM_MARKERS.weather)
    const editLabour = labourSection.indexOf(DIARY_EDIT_CORE_DOM_MARKERS.labour)
    const labourMountIdx = diaryPage.indexOf('<SiteDiaryLabourSection')
    const editVisitors = diaryPage.indexOf(DIARY_EDIT_CORE_DOM_MARKERS.visitors)
    const editHsHost = diaryPage.indexOf(DIARY_EDIT_CORE_DOM_MARKERS.hsHost)
    const dailyHs = dailySections.indexOf('title="H&S Incidents / Observations"')
    const dailyRfi = dailySections.indexOf('title="RFIs"')
    const dailyVo = dailySections.indexOf('title="Variations"')
    assert.ok(editWeather > 0 && editLabour >= 0)
    assert.ok(labourMountIdx > editWeather)
    assert.ok(editVisitors > labourMountIdx, 'Visitors must follow Labour on the workbench')
    assert.ok(editHsHost > editVisitors, 'H&S must follow Visitors on the workbench')
    assert.ok(dailyHs >= 0 && dailyRfi > dailyHs && dailyVo > dailyRfi)

    const savedWeather = savedDiaryPage.indexOf(DIARY_SAVED_REVIEW_CORE_DOM_MARKERS.weather)
    const savedLabour = savedDiaryPage.indexOf(DIARY_SAVED_REVIEW_CORE_DOM_MARKERS.labour)
    const savedVisitors = savedDiaryPage.indexOf('title="Visitors"')
    const savedHs = savedDiaryPage.indexOf(DIARY_SAVED_REVIEW_CORE_DOM_MARKERS.hs)
    assert.ok(savedWeather > 0 && savedLabour > savedWeather)
    assert.ok(savedVisitors > savedLabour, 'saved review Visitors must follow Labour')
    assert.ok(savedHs > savedVisitors, 'saved review H&S must follow Visitors')
    assert.doesNotMatch(savedDiaryPage, /title="Attendance Register"/)
    assert.deepEqual(DIARY_IN_APP_CORE_SECTION_ORDER.slice(0, 4), [
      'Weather',
      'Labour',
      'Visitors',
  ...
  it('Signature remains at the end of the Site Diary workbench', () => {
    ...
    assert.ok(labourMountIdx > 0 && signatureIdx > photoWsIdx)
    assert.ok(signatureIdx > labourMountIdx)
    assert.equal([...diaryPage.matchAll(/title="Cover photo"/g)].length, 0)
    assert.equal([...setupPage.matchAll(/title="Cover photo"/g)].length, 1)
  ...
  it('setup continue persists explicit Author Name to signed-in profile', () => {
    assert.match(setupBehaviour, /persistSignedInAuthorProfile/)
    assert.match(setupBehaviour, /authorName:\s*continueForm\.author/)
...
describe('14–17. Branding, plant isolation, Recent list, open write', () => {
  ...
  it('Share is raised on mobile with breathing room plus a chrome/safe-area floor', () => {
    assert.match(diaryPage, /className="zlog-diary-save-share-action"/)
    assert.match(diaryPage, /ref=\{saveCtaRef\}/)
    const saveCss = diaryPage.slice(
      diaryPage.indexOf('.zlog-diary-save-share-action {'),
      diaryPage.indexOf('`}</style>', diaryPage.indexOf('.zlog-diary-save-share-action {')),
    )
    assert.match(saveCss, /padding-bottom:\s*0/)
    assert.match(saveCss, /@media \(max-width:\s*768px\)/)
    assert.match(
      saveCss,
      /padding-bottom:\s*calc\(32px \+ max\(env\(safe-area-inset-bottom, 0px\), 40px\)\)/,
    )
    ...
    assert.match(saveBlock, /<PrimaryCTA/)
    assert.match(saveBlock, /surface="workbench"/)
    assert.match(saveBlock, /SAVE_CTA_IDLE_LABEL/)
    assert.match(saveBlock, /SAVE_CTA_SAVING_LABEL/)
    assert.doesNotMatch(saveBlock, /Save \/ Share/)
    assert.doesNotMatch(saveBlock, /#22c55e|POWDER_CTA_/)
    assert.doesNotMatch(saveBlock, /overflow(?:Y)?:\s*['"]?(?:auto|scroll)/)
    assert.doesNotMatch(saveBlock, /position:\s*['"](?:sticky|fixed)/)
  })
})
...
describe('Shift hydrate / defaults', () => {
  ...
  it('setup wires shift into continue form and draft create/update', () => {
    assert.match(setupBehaviour, /shift,/)
    assert.match(setupPage, /<option value="Day">Day<\/option>/)
    assert.match(setupBehaviour, /hydrateShift/)
    const continueSrc = readFileSync(join(root, 'lib/diary-setup-continue.js'), 'utf8')
...
describe('Back label on setup', () => {
  it('setup renders visible Back via shared ZlogBackControl (header + footer)', () => {
    assert.match(setupHostPage, /ZlogBackControl/)
    assert.match(
      setupHostPage,
      /const setupBackHref = useMemo\([\s\S]*?diaryHubHref\(\{ projectId: editingProjectId \}\)/,
    )
    assert.match(
      setupHostPage,
      /const setupBackNavigationHref = saving \? null : setupBackHref/,
    )
    assert.match(setupHostPage, /backHref=\{setupBackNavigationHref\}/)
    assert.match(setupHostPage, /href=\{setupBackNavigationHref\}/)
...
describe('saved-diary deletion (DIARY-029)', () => {
  ...
  it('saved list restores bulk Select; confirmed opened-diary Delete Diary remains', () => {
    assert.match(hubPage, /View Saved Diaries/)
    assert.doesNotMatch(hubPage, /Open to review/)
    assert.doesNotMatch(hubPage, /Use for Today/)
    ...
    assert.match(savedDiaryPage, /ReportDeletionDialog/)
    assert.match(savedDiaryPage, /deleteSiteDiaries/)
  ...
  it('uses compact record rows with a sticky contextual pane and Select browsing control', () => {
    ...
    assert.match(hubPage, /onClick=\{leaveSavedList\}/)
    assert.match(
      hubPage,
  ...
  it('opened diary keeps Share Report, Edit, and Use as Basis, with confirmed destructive Delete', () => {
    const pdfIndex = savedDiaryPage.indexOf('<PrimaryCTA')
    const editIndex = savedDiaryPage.indexOf('Edit This Diary')
    const basisIndex = savedDiaryPage.indexOf('Use as Basis for New Diary')
    ...
    assert.ok(deleteDivider > basisIndex && deleteDivider < deleteIndex)
    assert.match(savedDiaryPage, /Share Report/)
    assert.match(savedDiaryPage, /surface="workbench"/)
    assert.match(savedDiaryPage, /CopyPlus/)
    assert.match(savedDiaryPage, /<DestructiveButton/)
    assert.doesNotMatch(savedDiaryPage, /title="Saved Site Diary"/)
    assert.doesNotMatch(savedDiaryPage, /EqualChoiceButton/)
    assert.match(savedDiaryPage, /deleteActionStyle/)
    assert.match(savedDiaryPage, /createTodaysDiaryDraft/)
    assert.match(savedDiaryPage, /projectAndReportDetailsHref/)
    assert.match(savedDiaryPage, /stickyBack/)
    assert.doesNotMatch(savedDiaryPage, /Reviewing does not change this diary/)
    assert.doesNotMatch(savedDiaryPage, /Remove this saved diary/)
  })
