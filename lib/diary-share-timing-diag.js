...
export function formatShareTimingLines(snap) {
  ...
  const lines = [
    ...
    `Fetching: ${Number(c.pdfFetchAccumMs) || 0} ms accumulated (network)`,
    `Decode/bake: ${Number(c.pdfBakeAccumMs) || 0} ms accumulated (not wall)`,
