...
export function chunkReportIdsForDelete(ids) {
  const unique = [...new Set(
    (Array.isArray(ids) ? ids : [])
      .map((id) => String(id || '').trim())
      .filter(Boolean),
  ...
  if (unique.length < 1) throw new Error('Choose at least one saved report to delete.')
  const chunks = []
  for (let i = 0; i < unique.length; i += MAX_DELETE_COUNT) {
    chunks.push(unique.slice(i, i + MAX_DELETE_COUNT))
  }
  return chunks
}
...
/** Hub bulk-delete dialog title: "Delete 1 saved diary?" / "Delete 14 saved diaries?" */
export const BULK_SAVED_DIARY_DELETE_LABELS = {
  singular: '1 saved diary',
  plural: 'saved diaries',
}

export function selectedReportsCountLabel(count) {
  return `${Number(count) || 0} selected`
}
...
/**
 * User-facing bulk delete may exceed one RPC. Each call still goes through
 * deleteSiteDiaries / MAX_DELETE_COUNT. A later-chunk failure keeps already
 * confirmed IDs and returns the undeleted remainder.
 */
export async function deleteSiteDiariesInSafeBatches(supabase, reportIds) {
  const chunks = chunkReportIdsForDelete(reportIds)
  const deletedIds = []
  const seen = new Set()
  let cleanupPending = false
  try {
    for (const chunk of chunks) {
      const unused = chunk.filter((id) => !seen.has(id))
      if (!unused.length) continue
      const result = await deleteSiteDiaries(supabase, unused)
      for (const id of result.deletedIds.map(String)) {
        if (seen.has(id)) continue
        seen.add(id)
        deletedIds.push(id)
      }
      cleanupPending = Boolean(cleanupPending || result.cleanupPending)
    }
    return { ok: true, deletedIds, remainingIds: [], cleanupPending }
  } catch (errorCaught) {
    const remainingIds = chunks.flat().filter((id) => !seen.has(id))
    return {
      ok: false,
      deletedIds,
      remainingIds,
      cleanupPending,
      error: errorCaught?.message || 'We couldn’t delete the selected diaries. Try again.',
    }
  }
}
