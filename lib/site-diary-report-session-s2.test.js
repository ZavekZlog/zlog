...
describe('Site Diary structural S2 — cross-lifecycle publication safety', () => {
  it('publishReadSnapshot is only reachable via publishVerifiedViewerSnapshot', () => {
    const helperStart = viewPageSrc.indexOf('const publishVerifiedViewerSnapshot = useCallback')
    const depsClose = viewPageSrc.indexOf('[publishReadSnapshot],', helperStart)
    assert.ok(helperStart > 0 && depsClose > helperStart)
    const outside =
      viewPageSrc.slice(0, helperStart) +
      viewPageSrc.slice(depsClose + '[publishReadSnapshot],'.length)
    assert.equal((outside.match(/publishReadSnapshot\s*\(/g) || []).length, 0)
    assert.equal(
