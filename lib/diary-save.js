...
/**
 * Labour-only atomic save via finalize_site_diary_save (Apply / manual labour Save).
 * Does not run storage cleanup — RPC may return stale pending cleanup jobs for the report.
 *
 * @param {object} supabase
 * @param {string} reportId
 * @param {string} projectId
 * @param {object[]} labourPayload persist-shaped rows from labourFormToPersistRows
 * @returns {Promise<{ id: string, row: object|null, labourCount: number|null }>}
 */
export async function finalizeLabourOnly(supabase, reportId, projectId, labourPayload = []) {
  if (!reportId) {
    throw new DiarySaveError(
      'MISSING_REPORT_ID',
      'Cannot save labour: missing report id.',
      { stage: 'guard', mode: 'RPC_LABOUR_ONLY', reportId: null, projectId },
    )
  }
  if (!projectId) {
    throw new DiarySaveError('MISSING_PROJECT_ID', 'Cannot save labour: missing project id.', {
      stage: 'guard',
      mode: 'RPC_LABOUR_ONLY',
      reportId,
      projectId: null,
    })
  }

  const labour = Array.isArray(labourPayload) ? labourPayload : []
  ...
  if (error) {
    throw new DiarySaveError('FINALIZE_RPC_FAILED', error.message || 'Labour save failed.', {
      stage: 'rpc',
      mode: 'RPC_LABOUR_ONLY',
      reportId,
      projectId,
  ...
  if (!result.ok) {
    throw new DiarySaveError('FINALIZE_RPC_FAILED', 'Labour save did not succeed.', {
      stage: 'rpc',
      mode: 'RPC_LABOUR_ONLY',
      reportId,
      projectId,
  ...
  return {
    id: reportId,
    row: result.report ?? null,
    labourCount: result.labour_count ?? labour.length,
  }
}
