 ...
 */
async function updateDailyReportRow(supabase, { reportId, projectId, reportPayload, user }) {
  const execution = {
    mode: 'UPDATE',
...
export async function replaceLabour(supabase, reportId, labourPayload) {
...
export async function finalizeSiteDiarySave(supabase, {
  ...
  if (skipReport) {
    markShareTiming('finalize_report_start')
    row = baseline.reportRow
    diagnostic = {
      stage: 'update-skipped',
      ok: true,
      skipped: true,
      table: LIVE_DAILY_REPORTS.table,
      mode: 'UPDATE',
    ...
    const updated = await updateDailyReportRow(supabase, {
      reportId,
      projectId,
      reportPayload,
      user,
  ...
  await Promise.all([
    (async () => {
      ...
      if (!skipLabour) await replaceLabour(supabase, reportId, labourPayload)
      markShareTiming('finalize_labour_done')
    })(),
    (async () => {
      markShareTiming('finalize_plant_start')
      if (!skipPlant) await replacePlant(supabase, reportId, plantPayload)
