...
async function reconcilePhotos(supabase, {
  ...
  const { data: existingPhotoRows, error: listError } = await supabase
    .from(LIVE_REPORT_PHOTOS.table)
    .select('id, url')
    .eq('report_id', reportId)

  if (listError) throw new DiarySaveError('PHOTOS_LIST_FAILED', listError.message)
  ...
  for (const patch of updateExistingPhotos) {
    const fields = pickLiveColumns(patch.fields, LIVE_REPORT_PHOTOS.columns.filter((c) => c !== 'id' && c !== 'owner_id'))
  ...
  const rows = photoRecords.map((row) =>
    pickLiveColumns(row, LIVE_REPORT_PHOTOS.columns.filter((c) => c !== 'id')),
  )
  log('photos:insert', { count: rows.length, keys: rows[0] ? Object.keys(rows[0]) : [] })
  const { error: photosError } = await supabase.from(LIVE_REPORT_PHOTOS.table).insert(rows)
