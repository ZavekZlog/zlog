...
async function reconcilePhotos(supabase, {
  ...
  let existingPhotoRows
  {
    ...
    if (full.error && isMissingPreparedAssetColumnError(full.error)) {
      const basic = await supabase
        .from(LIVE_REPORT_PHOTOS.table)
        .select('id, url')
        .eq('report_id', reportId)
      if (basic.error) throw new DiarySaveError('PHOTOS_LIST_FAILED', basic.error.message)
      existingPhotoRows = basic.data
    } else if (full.error) {
      throw new DiarySaveError('PHOTOS_LIST_FAILED', full.error.message)
    } else {
      existingPhotoRows = full.data
    }
  }
...
export async function finalizeSiteDiarySave(supabase, {
  ...
  await Promise.all([
    replaceLabour(supabase, reportId, labourPayload),
    replacePlant(supabase, reportId, plantPayload),
  ])
