...
import { normalizeVisitorsRegisterProvenance } from './visitors-register-provenance.js'
...
/** Kept for copy-registry compatibility. Not shown on the workbench — a narrow daily_reports PATCH is not a whole-diary save. */
...
function stableVisitorsRegisterProvenance(rows) {
  return normalizeVisitorsRegisterProvenance(rows)
    .slice()
    .sort((a, b) => {
      const ka = `${a.evidencePath}|${a.sourceRow}`
      const kb = `${b.evidencePath}|${b.sourceRow}`
...
/**
 * A stale read may reconcile local form state only while the user is still
 * editing the payload that operation attempted. The server acknowledgement
 * remains valid bookkeeping when local state has advanced, but it must not
 * replace the newer local payload.
 */
export function shouldReconcileStaleAutosaveSnapshot(
  attemptedPayload,
  latestPayload,
  capturedLocalMutationRevision,
  currentLocalMutationRevision,
) {
  return capturedLocalMutationRevision === currentLocalMutationRevision
    && autosavePayloadsEqual(attemptedPayload, latestPayload)
}
...
export function pickDiaryAutosavePayload(input) {
  ...
  return {
    ...
    visitors_register_provenance: Object.prototype.hasOwnProperty.call(src, 'visitors_register_provenance')
      ? stableVisitorsRegisterProvenance(src.visitors_register_provenance)
      : [],
...
export function snapshotFromLiveRow(row) {
  ...
  return buildDiaryAutosavePayload({
    ...
    visitorsRegisterProvenance: live.visitors_register_provenance ?? [],
...
export function buildDiaryAutosavePayload({
  ...
  return pickDiaryAutosavePayload({
    ...
    visitors_register_provenance: stableVisitorsRegisterProvenance(visitorsRegisterProvenance),
...
export function autosaveStatusMessage(statusKind) {
  switch (statusKind) {
    ...
    case 'saved':
      // Legacy internal kind only — a narrow PATCH is not a whole-diary save.
      return null
    case 'network':
...
/** Exact string the workbench Save & Share strip may paint, or null. */
export function visibleDiaryAutosaveStatusCopy({
  error = '',
  saving = false,
  justSaved = false,
  autosaveStatus = null,
  ...
  if (!shouldShowDiaryAutosaveStatus({
    error,
    saving,
    justSaved,
    autosaveStatus,
  ...
  return autosaveStatusMessage(autosaveStatus) || null
}
...
export function diaryPersistenceUiPhase({
  ...
  if (autosaveStatus === 'saved') return 'idle'
  if (
...
export function shouldShowDiaryAutosaveStatus({
  ...
  if (autosaveStatus === 'saved') return false
  if (finalSaveInProgress || saving || justSaved || error) return false
  return true
}

/** Drop a generic autosave success claim. Pending/error statuses stay. */
export function dismissAutosaveSuccessStatus(status) {
...
function autosaveOwnerPart(value) {
  return String(value ?? '').trim()
}

export function diaryAutosaveOperationOwner({
  reportId = null,
  projectId = null,
  generation = 0,
} = {}) {
  return {
    reportId: autosaveOwnerPart(reportId),
    projectId: autosaveOwnerPart(projectId),
    generation: Number.isFinite(Number(generation)) ? Number(generation) : 0,
  }
}

export function diaryAutosaveOwnersEqual(left, right) {
  if (!left || !right) return false
  return (
    autosaveOwnerPart(left.reportId) === autosaveOwnerPart(right.reportId)
    && autosaveOwnerPart(left.projectId) === autosaveOwnerPart(right.projectId)
    && Number(left.generation) === Number(right.generation)
  )
}

function diaryAutosavePhysicalLaneKey(owner) {
  const normalized = diaryAutosaveOperationOwner(owner)
  return `${normalized.reportId}\u0000${normalized.projectId}`
}

let diaryAutosaveLifecycleGeneration = 0
const diaryAutosavePhysicalLanes = new Map()

export function nextDiaryAutosaveLifecycleGeneration() {
  diaryAutosaveLifecycleGeneration += 1
  return diaryAutosaveLifecycleGeneration
}

function waitForDiaryAutosavePhysicalLane(owner) {
  return diaryAutosavePhysicalLanes.get(diaryAutosavePhysicalLaneKey(owner))?.promise
    ?? Promise.resolve(null)
}

function enqueueDiaryAutosavePhysicalRequest(request) {
  const key = diaryAutosavePhysicalLaneKey(request.owner)
  const existing = diaryAutosavePhysicalLanes.get(key)
  if (existing) {
    existing.queued = request
    return existing.promise
  }

  const lane = { queued: null, promise: null }
  const drain = async (firstRequest) => {
    let current = firstRequest
    let result = null
    while (current) {
      result = await current.execute(current.owner, current.payload)
  ...
  const pending = drain(request).finally(() => {
    if (diaryAutosavePhysicalLanes.get(key) === lane) {
      diaryAutosavePhysicalLanes.delete(key)
    }
  })
  lane.promise = pending
  diaryAutosavePhysicalLanes.set(key, lane)
  return pending
}

/**
 * Component-local client authority over browser-module physical-report lanes.
 *
 * One report/project row remains single-flight across lifecycle generations
 * and Workbench instances. Different report rows remain independent.
 */
export function createDiaryAutosaveOperationQueue() {
  let activeOwner = null
  let highestLifecycleGeneration = Number.NEGATIVE_INFINITY

  return {
    setActive(owner) {
      const nextOwner = diaryAutosaveOperationOwner(owner)
      if (nextOwner.generation < highestLifecycleGeneration) return false
      if (
        nextOwner.generation === highestLifecycleGeneration
        && !diaryAutosaveOwnersEqual(nextOwner, activeOwner)
      ) {
        return false
      }
      highestLifecycleGeneration = nextOwner.generation
      activeOwner = nextOwner
      return true
    },
    clearActive(owner) {
      if (!diaryAutosaveOwnersEqual(owner, activeOwner)) return false
      activeOwner = null
      return true
    },
    waitFor(owner) {
      return waitForDiaryAutosavePhysicalLane(owner)
    },
    enqueue({ owner, payload, execute, commit }) {
      const operationOwner = diaryAutosaveOperationOwner(owner)
      const isCurrent = () => diaryAutosaveOwnersEqual(operationOwner, activeOwner)
      const request = {
        owner: operationOwner,
        payload,
        execute: (ownedOwner, ownedPayload) => {
          if (!isCurrent()) return Promise.resolve(null)
          return execute(ownedOwner, ownedPayload, { isCurrent })
        },
        commit: (result) => {
          if (isCurrent()) commit(result, operationOwner)
        },
      }
      return enqueueDiaryAutosavePhysicalRequest(request)
    },
  }
}
