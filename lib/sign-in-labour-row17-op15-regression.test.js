...
export function buildScanOperatives() {
...
export function buildAcceptedMergedBaseline(operatives) {
...
describe('row17 Architect must not re-expose op15 illegible after remove', () => {
  ...
  it('prune removes scan bucket when only operative moved to Visitors', () => {
    const operatives = [
      {
        id: 'arch',
        trade: 'Architect',
    ...
    const pruned = pruneDepletedSignInTradeHoursReviewRows(rows, operatives)
  ...
  it('move row17 architect to Visitors: no Decoration+Illegible double ownership', () => {
    ...
    assert.equal(
      moved.visitorsText,
      'Architect · 13:30–16:00 · Register row 17',
    )
    const duplicateAttempt = applyOperativeMoveToVisitorsFromReview({
    ...
    assert.equal(duplicateAttempt.reason, 'already-moved')
    assert.equal(
      duplicateAttempt.visitorsText,
      'Architect · 13:30–16:00 · Register row 17',
    )
    assert.equal(totalSignInTradeHoursReview(duplicateAttempt.reviewRows), 66.75)
    assert.equal(duplicateAttempt.reviewRows.length, 5)
  })
