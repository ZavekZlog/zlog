...
/**
 * Upload pending cover to immutable storage + DB when generation is still current.
 *
 * Prefers canonical prepared JPEG (`cover_processing_version` = pipeline id).
 * Falls back to raw bytes only when preparation cannot complete.
 *
...
export async function syncPendingCoverUpload(supabase, args = {}) {
  ...
  const persistCover = persistCoverFn
    || (uploadRawFn
      ? async (client, args) => {
        const rawUp = await uploadRawFn(client, args)
        return {
          storagePath: rawUp.storagePath,
          error: rawUp.error,
          prepared: false,
  ...
  let coverProcessingVersion = null
  let prepared = false
  try {
    const up = await persistCover(supabase, { userId, reportId, generation, file: uploadFile })
    ...
    coverProcessingVersion = up.coverProcessingVersion ?? null
    prepared = Boolean(up.prepared)
    mark('cover_upload_end', { ok: true, prepared })
  } catch {
    mark('cover_upload_end', { ok: false })
    mark('complete', { ok: false, reason: 'upload-failed' })
    return { ok: false, reason: 'upload-failed', storagePath: null }
  ...
  mark('complete', { ok: true, prepared })
  return {
    ok: true,
    storagePath,
    coverProcessingVersion,
