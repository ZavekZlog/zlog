...
describe('post-save Report Complete + no recent list on report page', () => {
  it('saving a diary hands off to Report Complete / Share', () => {
    assert.equal(
      postSaveDiaryHref('proj-1', 'rep-1'),
      '/dashboard/project/proj-1/diary/complete?report=rep-1',
    )
    assert.doesNotMatch(postSaveDiaryHref('proj-1', 'rep-1'), /edit=|compose=/)
    assert.match(postSaveDiaryHref('proj-1', 'rep-1'), /complete\?report=/)
  })
  ...
  it('diary page wires hydrate helpers and hides branding selector / recent list on report', () => {
    ...
    assert.match(page, /hydrateAuthorRole|hydratePlantFormRows/)
    assert.match(page, /prepareSiteDiaryPdf|shareSiteDiaryPdfNative/)
