...
export function diaryModeBanner({ mode, projectName = '' } = {}) {
  ...
  if (mode === 'edit') {
    return {
      ...
      text: name
        ? `You’re editing the saved Site Diary for ${name}. Make your changes, then tap Share when you’re ready.`
        : 'You’re editing this saved Site Diary. Make your changes, then tap Share when you’re ready.',
    }
  }
  return {
    kind: 'view',
