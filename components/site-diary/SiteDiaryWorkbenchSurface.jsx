...
export default function SiteDiaryWorkbenchSurface() {
  ...
  useEffect(() => {
    const reportId = searchParams.get('report') || searchParams.get('diaryId') || null
    const timing = readSavedDiaryEditNavTiming(reportId)
    emitShareDiag('site-diary-surface-workbench-mounted', {
      reportId,
      projectId,
      surface: 'workbench',
      hydrationSessionId: timing?.hydrationSessionId || null,
      elapsedMs:
        timing?.tapStartedAtMs != null
          ? Math.max(0, Date.now() - timing.tapStartedAtMs)
          : null,
    })
